...
(defun find-method-ref (class-sym name method-sym)
  (get-or-init (gethash method-sym (fvm-symbol-table *fvm*))
               (send-message :cref +callable-method+ (find-class-ref class-sym) (string name))))

(defun find-prop-get (class-sym name method-sym)
  (get-or-init (gethash method-sym (fvm-symbol-table *fvm*))
               (send-message :cref +callable-property-get+ (find-class-ref class-sym) (string name))))

(defun find-prop-set (class-sym name method-sym)
  (get-or-init (gethash method-sym (fvm-symbol-table *fvm*))
               (send-message :cref +callable-property-set+ (find-class-ref class-sym) (string name))))

(defun find-field-ref (class-sym name field-sym)
  (get-or-init (gethash field-sym (fvm-symbol-table *fvm*))
               (send-message :cref +callable-field+ (find-class-ref class-sym) (string name))))
...
(defun do-def-foil-class (full-class-name)
  (let ((bases (send-message :bases full-class-name))
    ...
    (multiple-value-bind (package) (split-package-and-class full-class-name)
      (let* ((class-sym (class-symbol full-class-name))
          ...
          ;build a path from the simple class symbol to the canonic
               ...
               `(setf (get ',class-sym :foil-info)
                      '((:member-kind . :type-name)
                        (:member-name . ,(intern full-class-name))
                        (:bases . ,(mapcar #'symbol-for bases))))
  ...
  (when ctor-list
    (let ((ctor-sym (constructor-symbol full-class-name)))
      `((defun ,ctor-sym (&rest args)
        ...
        (setf (get ',ctor-sym :foil-info)
                      '((:member-kind . :ctor)
                        (:member-name . ,(intern full-class-name))
                        (:signatures . ,(mapcar (lambda (sig)
                                                  (cons (intern full-class-name)
                                                        (mapcar #'symbol-for sig)))
                                                ctor-list))))
    ...
    (maphash
     (lambda (method-sym methods)
       (let* ((name (find-tag-atom :name (first methods)))
              ...
              (member-of-sym (intern full-class-name)))
         (push (if is-static
         ...
         (push `(setf (get ',method-sym :foil-info)
                      '((:member-of . ,member-of-sym)
                        (:member-kind . :method)
                        (:is-static . ,is-static)
                        (:member-name . ,(intern name))
                        (:signatures . ,(mapcar (lambda (m)
                                                  (let ((sig (mapcar #'symbol-for (find-tag-atom :sig m))))
                                                    (if is-static
                                                        sig
                                                      ;splice 'this' into sig
                                                      (list* (car sig) member-of-sym (rest sig)))))
                                                methods))))
               defs)))
     methods-by-sym)
    (nreverse defs)))
  ...
  (let* ((defs nil))
    (dolist (field fields)
      (let* ((is-static (find-tag-atom :static field))
             ...
             (field-type-sym (symbol-for (find-tag-atom :type field)))
             (member-of-sym (intern full-class-name))
        ...
        (push `(setf (get ',field-sym :foil-info)
                      '((:member-of . ,member-of-sym)
                        (:member-kind . :field)
                        (:is-static . ,is-static)
                        (:sig . ,(if is-static (list field-type-sym) (list field-type-sym member-of-sym)))
                        (:member-name . ,(intern field-name))
                        (:type . ,field-type-sym)))
               defs)
        (unless is-const
          (push `(setf (get ',setter-sym :foil-info)
                       '((:member-of . ,member-of-sym)
                         (:member-kind . :field-setter)
                         (:is-static . ,is-static)
                         (:sig . ,(if is-static (list :void field-type-sym)
                                    (list :void member-of-sym field-type-sym)))
                         (:member-name . ,(intern field-name))
                         (:type . ,field-type-sym)))
                defs))))
    (nreverse defs)))

(defun call-prop-get (class-sym name method-sym this &rest args)
  (with-vm-of this
;    (format t "prop-get ~A meth ~A~%" name method-sym)
    (let ((cref (find-prop-get class-sym name method-sym)))
      (apply #'send-message :call cref *marshalling-flags* *marshalling-depth* this args))))

(defun call-prop-set (class-sym name method-sym this &rest args)
  (with-vm-of this
  ...
  (let ((defs nil))
    (dolist (prop props)
      (let* ((is-static (find-tag-atom :static prop))
             ...
             (prop-type-sym (symbol-for (find-tag-atom :type prop)))
             (member-of-sym (intern full-class-name))
         ...
         (when get-function
             ...
             (push `(setf (get ',method-sym :foil-info)
                      '((:member-of . ,member-of-sym)
                        ...
                        (:sig . ,(if is-static (list prop-type-sym) (list prop-type-sym member-of-sym)))
                        (:is-static . ,is-static)
                        (:member-name . ,(intern get-function))
                        (:type . ,(symbol-for (find-tag-atom :type prop)))))
               defs))
         (when set-function
             (push (if is-static
             ...
             (push `(setf (get ',setter-sym :foil-info)
                      '((:member-of . ,(intern full-class-name))
                        (:member-kind . :property-setter)
                        (:is-static . ,is-static)
                        (:sig . ,(if is-static (list :void prop-type-sym)
                                    (list :void member-of-sym prop-type-sym)))
                        (:member-name . ,(intern set-function))
                        (:type . ,(symbol-for (find-tag-atom :type prop)))))
               defs))
