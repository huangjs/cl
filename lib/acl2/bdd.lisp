...
#|
 (let ((new-mx-id (1+mx-id mx-id)))
   (declare (type (signed-byte 29) new-mx-id))
...
; "turnaround" here earlier.  But we have not yet checked this claim carefully.
  ...
  (declare (type (signed-byte 29) i acc)
...
; fixnum for it in GCL.
  ...
  (declare (type (signed-byte 29) op-code))
  (the-mv
   2
   (signed-byte 29)
...
(defun op-hash-index-string (index acc string)
  (declare (type (signed-byte 29) index acc))
  (the-fixnum
   (cond
    ((= index 0) acc)
    (t (let ((index (1- (the-fixnum index))))
         (declare (type (signed-byte 29) index))
...
(defun bdd-quotep (term op-ht mx-id)
  (declare (type (signed-byte 29) mx-id))
  (the-mv
   3
   (signed-byte 29)
   (cond
    ...
    (t
     (mv-let (hash-index ans)
             ...
             (declare (type (signed-byte 29) hash-index))
             (cond
...
; two hash arrays.
  ...
  (declare (type (signed-byte 29) n mx-id))
  (the-mv
   4
   (signed-byte 29)
...
; terms as false-branch-args.
  ...
  (declare (type (signed-byte 29) var-id))
  (if (endp args)
      (mv nil nil)
    (mv-let (x y)
...
; that op is not a member of bdd-constructors.
  ...
  (declare (type (signed-byte 29) hash-index mx-id))
  (the-mv
   5
   (signed-byte 29)
   (let ((new-mx-id (1+mx-id mx-id))
...
(defun bdd-ev-fncall
  ...
  (the-mv
   ...
   (mv-let (erp val latches)
           (ev-fncall op (cst-list-to-evg-list args) state nil nil)
...
; Keep this case in sync with make-if.
    ...
    ((let ((true-split-var (split-var true-cst))
       ...
       (declare (type (signed-byte 29) test-id))
...
; combine-op-csts-comm.
  ...
  (declare (type (signed-byte 29) op-code mx-id))
  (the-mv
   5
   (signed-byte 29)
   (mv-let
...
; are all the bdd rules that rewrite calls of the function symbol op.
  ...
  (declare (type (signed-byte 29) op-code mx-id hash-index))
  (the-mv
   5
   (signed-byte 29)
   (mv-let
...
; We need a cst next, though we don't care about it.
    ...
    (t (mv-let
        ...
        (declare (type (signed-byte 29) opcode))
        (cond
         (comm-p
          (mv-let?
...
(deflabel bdd-algorithm

; Note:  the ``IF-lifting-for-IF loop'' described here is really
; combine-if-csts+.
  ...
  (A2) ~em[BDD-constructors] (assumed to be ~c['(cons)]) and ~em[BDD terms]
  ...
  ~il[acl2-defaults-table] (~pl[acl2-defaults-table]) or by
  supplying a ~c[:BDD-CONSTRUCTORS] hint (~pl[hints]).  Warning:
  ...
  we assume that BDD-constructors is ~c['(cons)].
  ...
  hypothesis, since the ACL2 axiom ~c[CONS-EQUAL] (which you can
  ...
  A rule of class ~c[:]~ilc[rewrite] (~pl[rule-classes]) is said to be
  ...
  (4) Its ~c[:]~ilc[loop-stopper] field is ~c[nil], i.e., it is not a
  ...
  (~pl[rule-classes]) is said to be a ``~il[BDD] definition rule''
  ...
  However, ACL2 uses a number of tricks, including ~il[type-set]
  ...
  Whenever we apply IF-lifting, we do so for the available ~c[test]
  ...
  return ~c[(IF test tbr fbr)].  Otherwise, we apply IF-lifting to
  ...
  ~c[z] by ~c[nil] before recursively applying IF-lifting-for-IF.
  ...
  the ~c[:]~ilc[executable-counterpart] of ~c[f] is enabled, then the
  ...
  BDD-constructor, then we return ~c[(f x1'... xn')].  Otherwise, if a
  ...
  IF-lifting, then we return ~c[(f x1'... xn')].  Otherwise we apply
  IF-lifting to ~c[(f x1'... xn')] to obtain a term ~c[(IF test tbr fbr)];
  ...
  ~c[CONS] is the only BDD-constructor.
  ...
  algorithm on a top-level term ~c[t0] is a term ~c[u0] that contains
  ...
  ~c[boolean-listp-forward] (you can see it using ~c[:]~ilc[pe]) to try to
  ...
  a theory (~pl[theories]) in which all relevant non-recursive
  ...
  the rules ~c[append-cons] and ~c[append-nil] in the documentation for
  ...
  1-dimensional array.  ~l[arrays].  In addition, we use a second
...
(deflabel bdd-introduction
  ...
  ******** FAILED ********  See :DOC failure  ******** FAILED ********
  ...
  ~c[show-bdd] to see what went wrong, and use the resulting
  ...
  bound to the 4-element list ~c[(list x0 x1 x2 x3)].
  ~bv[]
  ACL2 !>(thm
...
static object  LnkTLI273(va_alist)va_dcl{va_list ap;va_start(ap);return(object )call_proc(VV[273],&LnkLI273,2,ap);} /* DECODE-CST-ALIST */

static object  LnkTLI253(va_alist)va_dcl{va_list ap;va_start(ap);return(object )call_proc(VV[253],&LnkLI253,2,ap);} /* LEAF-CST-LIST-ARRAY */
