...
(defun polys-from-type-set (term force-flag dwp type-alist ancestors
                                 ens wrld ttree)
...
; normalize-poly-lst, and thereby avoid the potential error situation.
  ...
  (if (good-pot-varp term)
    (mv-let (ts ts-ttree)
      (type-set term force-flag dwp type-alist ancestors ens wrld ttree
                nil nil)
         ...
         ((inequalityp atm)
          (let ((lhs (fargn atm 1))
            ...
            (mv-let (ts-lhs ts-ttree)
              (type-set lhs force-flg nil
                        type-alist nil ens wrld ttree
                        nil nil)
              (mv-let (ts-rhs ts-ttree)
                (type-set rhs force-flg nil
                          type-alist nil ens wrld ts-ttree
                          nil nil)
...
;          (and (<= lhs rhs) (<= rhs lhs)))
                    ...
                    (let ((rationalp-flg (and (ts-rationalp ts-lhs)
                                              (ts-rationalp ts-rhs))))
                      (poly-set 'and
                                (add-linear-terms
                                 :lhs lhs
  ...
  (cond ((null term-lst)
        ...
        (t (let ((lst (linearize (car term-lst)
             ...
             (mv-let (poly-lst split-lst)
               (linearize-lst (cdr term-lst)
                              (cdr ttrees)
                              positivep
                              type-alist ens force-flg wrld state)
               (cond
                ((null lst) (mv poly-lst split-lst))
                ((null (cdr lst))
                 (mv (append (car lst) poly-lst)
                     split-lst))
                (t (mv poly-lst (cons lst split-lst)))))))))
