...
(defun cons-tag-trees (tree1 tree2)
...
; 'accumulated-into-state tag with (irrelevant, non-nil) value t at the top of
...
; "Slow" or "Smart" cons-tag-trees.  Unions together the two trees but
; only conses those tagged values in ttree1 that are not already in
; ttree2.

  (cond ((null ttree1) ttree2)
        ((symbolp (caar ttree1))
         (scons-tag-trees (cdr ttree1)
                          (add-to-tag-tree (caar ttree1)
                                           (cdar ttree1)
                                           ttree2)))
        (t (scons-tag-trees (cdr ttree1)
                            (scons-tag-trees (car ttree1) ttree2)))))

(defun move-unmarked-subbtrees (ttree1 ttree2)

; This function i like scons-tag-trees, except that it ignores subtrees of
; ttree1 tagged with 'accumulated-into-state, which presumably are already
; included in ttree2.

  (cond
   ((null ttree1) ttree2)
   ((symbolp (caar ttree1))
    (cond ((eq (caar ttree1) 'accumulated-into-state)
           ttree2)
          (t (move-unmarked-subbtrees (cdr ttree1)
                                      (add-to-tag-tree (caar ttree1)
                                                       (cdar ttree1)
                                                       ttree2)))))
   (t
    (move-unmarked-subbtrees (cdr ttree1)
                             (move-unmarked-subbtrees (car ttree1) ttree2)))))

(defun reverse-marked-tag-tree-to-list (ttree acc)

; We create a list without duplicates, whose elements are the same as those in
; the given tag tree as though 'accumulated-ttree marks had been removed from
; ttree.  Moreover, if x occurs before y in ttree, then y occurs before x in
; the returned list.

  (cond ((null ttree) acc)
        ((symbolp (caar ttree))
         (cond ((eq (caar ttree) 'accumulated-into-state)
                (reverse-marked-tag-tree-to-list (cdr ttree) acc))
               (t
                (reverse-marked-tag-tree-to-list (cdr ttree)
                                                 (add-to-set-equal (car ttree) acc)))))
        (t (reverse-marked-tag-tree-to-list
            (cdr ttree)
            (reverse-marked-tag-tree-to-list (car ttree) acc)))))
...
; that we mark the returned ttree as having been accumulated.
  ...
  (pprogn
   (f-put-global 'accumulated-ttree
                 (move-unmarked-subbtrees ttree
                                          (f-get-global 'accumulated-ttree state))
                 state)
   (value (cons (cons 'accumulated-into-state t)
...
; ordered-symbol-alistp-remove-first-pair-test in the :mini-proveall fail,
; probably because the order of elements is relevant to new proof-checker
; subgoals resulting from a call of remove-byes-from-tag-tree.

                (reverse (reverse-marked-tag-tree-to-list ttree nil))))))
...
(defun remove-ttrees-from-type-alist (type-alist)
...
; force-assumption.
  ...
  (cond
   ...
   ((contains-assumptionp (cddar type-alist))
    (remove-ttrees-from-type-alist (cdr type-alist)))
   (t (cons (list* (caar type-alist)
                   (cadar type-alist)
                   nil)
            (remove-ttrees-from-type-alist (cdr type-alist))))))
...
; type-alist!  We first remove from type-alist all the entries depending upon
; assumptions.  Then we delete all ttrees from the type-alist.  It is
; legitimate to throw away any hypothesis, thus we can delete the entries we
; choose.  Why do we throw out the type-alist entries depending on assumptions?
; The reason is that in the forcing round we actually generate a formula
; representing (implies type-alist' term) and this formula does not encode the
...
; in ok-to-force.
  ...
  (let ((type-alist (remove-ttrees-from-type-alist type-alist)))
    (cond
     ((not force-flg)
      (mv force-flg
          (er hard 'force-assumption
...
(defun fn-count-evg-rec (evg acc)
...
; counts the size of evgs.
  ...
  (cond ((atom evg)
         (cond ((rationalp evg)
               ...
               ((realp evg) (er hard 'fn-count-evg 
               ...
               #+:non-standard-analysis
               ((complexp evg) (er hard 'fn-count-evg 
                                   "Encountered a complex irrational in fn-count-evg!"))

               ((symbolp evg)
                (+ 2 (* 2 (length (symbol-name evg))) acc))
               ((stringp evg)
                (+ 1 (* 2 (length evg)) acc))
               (t ; (characterp evg)
                (1+ acc))))
...
(defmacro fn-count-evg (evg)
  `(fn-count-evg-rec ,evg 0))

(mutual-recursion

(defun arith-fn-var-count (term times-flag)
  ...
  (cond ((variablep term)
        ...
        ((fquotep term)
         (mv 0 0))
        (t (mv-let (f v)
             (arith-fn-var-count-lst (fargs term)
                                     (eq (ffn-symb term) 'BINARY-*))
             (mv (if (and times-flag
                          (eq (ffn-symb term)
                              'UNARY-/))
                     f
                   (+ 1 f))
...
(defun arith-fn-var-count-lst (lst times-flag)
  (declare (xargs :guard (pseudo-term-listp lst)))
  (cond ((null lst)
         (mv 0 0))
        (t (mv-let (f1 v1)
             (arith-fn-var-count (car lst) times-flag)
             (mv-let (f2 v2)
               (arith-fn-var-count-lst (cdr lst) times-flag)
               (mv (+ f1 f2) (+ v1 v2)))))))

)

(defun arith-term-order (term1 term2)
  (mv-let (fn-count1 var-count1)
    (arith-fn-var-count term1 nil)
    (mv-let (fn-count2 var-count2)
      (arith-fn-var-count term2 nil)
...
; term and ki is a rationalp.  The alist is kept ordered by arith-term-order
...
; The pt tag indicates literals of current-clause used in the production
; of the poly.

; The parents field is a list of parents and is set-eql to the union
...
; literal.  (This used to be done by to-be-ignoredp, which used to
; take up to 80% of the time spent by add-poly.)  See collect-parents
; and marry-parents for how we establish and maintain this
; relationship, and ignore-polyp for its use.

; Rational-poly-p is a booolean flag used in non-linear arithmetic.  When it
; is true, then the right-hand side of the inequality (the polynomial) is
...
; When rational-poly-p is true we know that the product of two positive polys
; is also positive.

; Derived-from-not-equalityp keeps track of whether the poly in
; question was derived directly from a top-level negated equality.
; This field is new to v2_8 --- previously its value was calculated
; as needed.  In the rest of this comment, we address two issues ---
; (1) What derived-from-not-equalityp is used for.
; (2) Differences with earlier behavior.

; (1) What derived-from-not-equalityp is used for: In
; process-equational-polys, we scan through the
; simplify-clause-pot-lst and look for complementary pairs of
...
; process-equational-polys (or rather its sub-functions), and the
; result is used to prune the list of candidate inequalities.

; (2) Differences with earlier behavior:
; Previously, the function descends-from-not-equalityp played the role
; of the new field :derived-from-not-equalityp.  This function was
...
(defun poly-weakerp (poly1 poly2)
...
(defun bounds-polys-with-var1 (poly-lst pt)

; We cdr down poly-lst, looking for a bounds poly.  Poly-lst 
...
; in poly-lst because we filter out any ones that are weaker than
; one already present with poly-member before adding it.
  ...
  (cond ((null poly-lst)
        ...
        ((and (null (cdr (access poly (car poly-lst) :alist)))
         ...
         (list (car poly-lst)))
        (t
         (bounds-polys-with-var1 (cdr poly-lst) pt))))
...
; with var.  If there are no such polys, we return nil.
  ...
  (cond ((null pot-lst) nil)
        ((equal var (access linear-pot (car pot-lst) :var))
         (append (bounds-polys-with-var1
                  (access linear-pot (car pot-lst) :negatives) pt)
                 (bounds-polys-with-var1
                  (access linear-pot (car pot-lst) :positives) pt)))
...
(defun new-vars-in-pot-lst (new-pot-lst old-pot-lst)

; We return all the new vars of new-pot-lst.  A "var" of a pot-lst is
; the :var component of a linear-pot in the pot-lst.  A var is
; considered "new" if it is not a variablep (i.e., is a function
; application) and the var is not a var of the old-pot-lst.

; New-pot-lst is an extension of old-pot-lst, obtained by successive
; calls of add-poly.  Every variable of old-pot-lst is in the new, but
...
; for a pot with a variablep :var.  Consider an old-pot-lst with one
; pot, (foo x), and a new-pot-lst with two pots, x and (foo x).
; Previously, since (variablep (access linear-pot (car new-pot-lst)
...
|#
        ...
        ((or (null old-pot-lst)
             (not (equal (access linear-pot (car new-pot-lst) :var)
                         (access linear-pot (car old-pot-lst) :var))))
         (if (not (variablep (access linear-pot (car new-pot-lst) :var)))
             (cons (access linear-pot (car new-pot-lst) :var)
                   (new-vars-in-pot-lst (cdr new-pot-lst) old-pot-lst))
           (new-vars-in-pot-lst (cdr new-pot-lst) old-pot-lst)))
...
#|
  ...
  (cond
   ((time-limit4-reached-p "Out of time in add-poly.") ; nil, or throws
    (mv nil nil nil))
   ((or (null pot-lst)
