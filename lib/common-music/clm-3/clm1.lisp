...
#+openmcl (defun c-seek (fd loc type) (ccl::external-call "_lseek" :signed fd :signed loc :signed type :void))
...
#+openmcl (defun c-read-floats (fd arr n)
	    (ccl::with-foreign-double-float-array-to-c-and-lisp (p arr)
	      (ccl::external-call "_read" :signed fd :address p :signed n :signed)))
...
#+openmcl (defun c-read-ints (fd arr n) (ccl::external-call "_read" :signed fd :address (heap-int* arr) :signed n :signed))
...
#+openmcl (defun c-write-floats (fd arr n)
	    (ccl::with-foreign-double-float-array-to-c-and-lisp (p arr)
	      (ccl::external-call "_write" :signed fd :address p :signed n :signed)))
...
#+openmcl (defun c-write-ints (fd arr n) (ccl::external-call "_write" :signed fd :address (heap-int* arr) :signed n :signed))
...
#+openmcl (defun c-close (fd) (ccl::external-call "_close" :signed fd :void))
...
#+openmcl (defun c-open-input-file (file)
	    (ccl:with-cstrs ((f file))
	      (ccl::external-call "_mus_file_open_read" :address f :signed)))
...
#+openmcl (defun c-open-output-file (file)
	    (ccl:with-cstrs ((f file))
	      (ccl::external-call "_mus_file_open_write" :address f :signed)))
...
#+openmcl (defun c-create-file (file)
	    (ccl:with-cstrs ((f file))
	      (ccl::external-call "_mus_file_create" :address f :signed)))
...
#+openmcl (defun clm-swap-ints (arr n) (ccl::external-call "_swap_int_array" :address (heap-int* arr) :signed n :void))
...
#+openmcl (defun clm-swap-doubles (arr n)
	    (ccl::with-foreign-double-float-array-to-c-and-lisp (p arr)
   	      (ccl::external-call "_swap_double_array" :address p :signed n :void)))
...
#+clisp (defun clm-swap-doubles (arr n) (warn "clm-swap-doubles doesn't work in clisp") -1)
