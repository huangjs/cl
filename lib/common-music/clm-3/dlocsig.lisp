...
(eval-when (compile)
  (let* ((current *package*))
    (in-package :clm)
    (export '(clm::def-optkey-fun clm::x-norm))
  ...
  (let* (;; number of output channels
	...
	(setf (aref out-delays channel) (if (/= delay 0.0) (make-delay (time->samples delay)) nil)
      ...
      (make-move-sound
       ...
       (let ((d
       (make-dlocs :start (floor start)
		   :end (floor (* (+ start-time duration) *srate*))
		   :out-channels (floor (speaker-config-number speakers))
		   :rev-channels (floor rev-channels)
...
;;;	 (format t "dlocs: start ~A, end: ~A, out: ~A, rev: ~A~%    path: ~A~%    delay: ~A~%    rev: ~A~%    out-delays: ~A~%    gains: ~A~%    rev-gains: ~A~%    out-amp: ~A~%"
;;;		     (dlocs-start d) (dlocs-end d) (dlocs-out-channels d) (dlocs-rev-channels d) 
;;;		     (dlocs-path d) (dlocs-delay d) (dlocs-rev d) (dlocs-out-delays d) (dlocs-gains d) (dlocs-rev-gains d) (dlocs-out-map d))
