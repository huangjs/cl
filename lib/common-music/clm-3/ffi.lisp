...
;;; foreign function interfaces for ACL, CMUCL, SBCL, OpenMCL, Clisp
...
(defun sound-chans (name) (mus-sound-chans (fullname name)))
(defun sound-duration (name) (mus-sound-duration (fullname name)))
(defun sound-data-format (name) (mus-sound-data-format (fullname name)))
(defun sound-data-location (name) (mus-sound-data-location (fullname name)))
(defun sound-datum-size (name) (mus-sound-datum-size (fullname name)))
(defun sound-header-type (name) (mus-sound-header-type (fullname name)))
(defun sound-length (name) (mus-sound-length (fullname name)))
(defun sound-samples (name) (mus-sound-samples (fullname name)))
(defun sound-frames (name) (mus-sound-frames (fullname name)))
(defun sound-srate (name) (mus-sound-srate (fullname name)))
(defun sound-comment (name) (mus-sound-comment (fullname name)))
...
(defun describe-audio () (mus-audio-describe))
...
(defun read-audio-state (a b c d) (mus-audio-mixer-read a b c d))
(defun write-audio-state (a b c d) (mus-audio-mixer-write a b c d))
...
#+(and excl acl-50)
  (progn
    ...
    (ff:def-foreign-call (sl-dac-1 "sl_dac")
			 ...
			 :returning :int)
    #-(or x86-64 windoze) (ff:def-foreign-call (clm-send-snd "clm_send_snd")
				   ((command (* :char) #-acl-60 string))
				   #+acl-61 :strings-convert #+acl-61 t 
				   :returning :int)
    #-(or x86-64 windoze) (ff:def-foreign-call (clm-start-snd "clm_start_snd") (:void) :returning :int)
    #-(or x86-64 windoze) (ff:def-foreign-call (clm-init-x "clm_init_x")
				   ((command (* :char) #-acl-60 string))
				   #+acl-61 :strings-convert #+acl-61 t 
				   :returning :void)
    #-(or acl-60 windoze x86-64) (ff:def-foreign-call (clm-receive-snd-1 "clm_receive_snd") (:void) :returning :int)
    #+(and acl-60 (not windoze) (not x86-64)) (ff:def-foreign-call (clm-receive-snd "clm_receive_snd") (:void) :returning ((* :char)))
    ...
    ;; cmus.c
    #-acl-70
    (ff:def-foreign-call (basic-convolve "mus_convolution")
    ...
    (ff:def-foreign-call (array->file "clm_array2file")
     ...
     (ff:def-foreign-call (clm-mix "clm_mix")
 			 ((outfile (* :char) #-acl-60 string) (infile (* :char) #-acl-60 string)
 			  (out_start :int) (out_frames :int) (in_start :int))
 			 #+acl-61 :strings-convert #+acl-61 t 
 			 :returning :void)
    ...
    (ff:def-foreign-call (mus-file-buffer-size "mus_file_buffer_size") (:void) :returning :int)
    (ff:def-foreign-call (mus-set-file-buffer-size "mus_set_file_buffer_size") ((clipped :int)) :returning :int)    
   )
...
(progn
    ...
    #-(or x86-64 netbsd windoze) (def-alien-routine ("clm_send_snd" clm-send-snd) c-call:int (command c-call:c-string))
    #-(or x86-64 netbsd windoze) (def-alien-routine ("clm_start_snd" clm-start-snd) c-call:int)
    #-(or x86-64 netbsd windoze) (def-alien-routine ("clm_init_x" clm-init-x) c-call:void (command c-call:c-string))
    #-(or x86-64 netbsd windoze) (def-alien-routine ("clm_receive_snd" clm-receive-snd) c-call:c-string)
    ...
    ;; cmus.c
    (def-alien-routine ("mus_convolution" basic-convolve-1) c-call:void (rl1 (* double-float)) (im1 (* double-float)) (len c-call:int))
    (defun basic-convolve (rl1 im1 len) (basic-convolve-1 (array-data-address rl1) (array-data-address im1) len))
    ...
    (def-alien-routine ("clm_mix" clm-mix) 
 	c-call:void
 	(outfile c-call:c-string) (infile c-call:c-string)  (out_start c-call:int) (out_frames c-call:int) (in_start c-call:int))
    ...
    (def-alien-routine ("mus_file_buffer_size" mus-file-buffer-size) c-call:int)
    (def-alien-routine ("mus_set_file_buffer_size" mus-set-file-buffer-size) c-call:int (val c-call:int))
...
(progn
    ...
    #-(or x86-64 netbsd windoze) (sb-alien:define-alien-routine ("clm_send_snd" clm-send-snd) sb-alien:int (command sb-alien:c-string))
    #-(or x86-64 netbsd windoze) (sb-alien:define-alien-routine ("clm_start_snd" clm-start-snd) sb-alien:int)
    #-(or x86-64 netbsd windoze) (sb-alien:define-alien-routine ("clm_init_x" clm-init-x) sb-alien:void (command sb-alien:c-string))
    #-(or x86-64 netbsd windoze) (sb-alien:define-alien-routine ("clm_receive_snd" clm-receive-snd) sb-alien:c-string)
    ...
    (defun sound-maxamp (file chans arr times) (sound-maxamp-1 file chans (array-data-address arr) (array-data-address times)))
    ;; cmus.c
    (sb-alien:define-alien-routine ("mus_convolution" basic-convolve-1) sb-alien:void
	...
	sb-alien:void
 	(outfile sb-alien:c-string) (infile sb-alien:c-string)  (out_start sb-alien:int) (out_frames sb-alien:int) (in_start sb-alien:int))
    ...
    (sb-alien:define-alien-routine ("mus_file_buffer_size" mus-file-buffer-size) sb-alien:int)
    (sb-alien:define-alien-routine ("mus_set_file_buffer_size" mus-set-file-buffer-size) sb-alien:int (val sb-alien:int))
...
(progn
  ...
  (defun get-cstring (str)
    (when (not (ccl:%null-ptr-p str))
      (%get-cstring str)))
  ...
  (defun basic-convolve (rl im len)
    (ccl::with-foreign-double-float-array-to-c-and-lisp (arl rl)
      (ccl::with-foreign-double-float-array-to-c-and-lisp (aim im)
  ...
  (defun clm-mix (ofile ifile ostart oframes istart)
     (ccl:with-cstrs ((f1 ofile) (f2 ifile))
       (ccl::external-call "_clm_mix"
 			 :address f1 :address f2 :signed ostart :signed oframes :signed istart
 			 :void)))
  ;; these courtesy of Todd Ingalls
  #+X11
  (defun clm-send-snd (command)
    (ccl:with-cstrs ((cmd command))
      (ccl::external-call "_clm_send_snd" :address cmd :signed)))
  #+X11
  (defun clm-start-snd () (ccl::external-call "_clm_start_snd" :signed))
  #+X11
  (defun clm-init-x (command)
    (ccl:with-cstrs ((cmd command)) (ccl::external-call "_clm_init_x" :address cmd :void)))
  #+X11
  (defun clm-receive-snd ()
    (ccl:%get-cstring
...
(progn
    ...
    (ffi:def-call-out clm-mix (:name "clm_mix") (:library (full-lib-name)) (:return-type ffi:nil)
      (:arguments (outfile ffi:c-string) (infile ffi:c-string)  (out_start ffi:int) (out_frames ffi:int) (in_start ffi:int)))
    ...
    (ffi:def-call-out mus-file-buffer-size (:name "mus_file_buffer_size") (:library (full-lib-name)) (:return-type ffi:int))
    (ffi:def-call-out mus-set-file-buffer-size (:name "mus_set_file_buffer_size") (:library (full-lib-name)) (:return-type ffi:int) 
...
;    (ffi:def-call-out basic-convolve-1 (:name "mus_convolution") (:library (full-lib-name)) (:return-type ffi:nil) 
...
;      (:arguments (rl (ffi:c-array-ptr ffi:double-float)) (im (ffi:c-array-ptr ffi:double-float)) (len ffi:int) (isign ffi:int)))
    ...
    (defun basic-convolve (rl1 im1 len) (warn "basic-convolve doesn't work in clisp"))
    (defun clm-fft (rl im len isign) (warn "clm-fft doesn't work in clisp"))
