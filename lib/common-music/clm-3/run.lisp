...
;;; __float_didf trouble in Sun [sbcl makes libclm.so -- can it be used with acl/cmucl?]
...
(defvar setf-functions (list (list 'mus-frequency '<mus-set-frequency> nil :mus-any)
			     ...
			     (list 'mus-formant-radius '<mus-set-formant-radius> nil :mus-any 'formant)
			     ...
			     (list 'mus-cosines '<mus-set-cosines> nil :mus-any)
			     ...
			     (list 'phase-vocoder-outctr '<phase-vocoder-set-outctr> nil :mus-any 'phase-vocoder)))
...
(defun mark-as-frame (val num)
  ...
  (if (listp val)
	...
	(when (and info
	  ...
	  (setf (varinfo-gen-type info) 'frame)))))

(defun mark-as-frame2frame (val num)
  (if (listp val)
      (let ((info (gethash (second val) vars)))
	(when (and info
		   (not (varinfo-type info))
	  ...
	  (if (= num 1)
	      (progn
		(if *clm-debug* (format t "set ~A as mixer~%" (second val)))
		(setf (varinfo-gen-type info) 'mixer))
	    (if (= num 2)
		(progn
		  (if *clm-debug* (format t "set ~A as frame~%" (second val)))
		  (setf (varinfo-gen-type info) 'frame))))))))

(defun mark-as-mixer (val num)
  (declare (ignore num))
  (if (listp val)
      (let ((info (gethash (second val) vars)))
	(when (and info
...
(def-clm-fun 'sine-summation    #'(lambda (var x) (gen-package-op '<sine-summation> var x :clm-real 'sine-summation)))
(def-clm-fun 'sine-summation?   #'(lambda (var x) (gen-package-op '<sine-summation?> var x :clm-boolean :mus-any)))
							   ...
							   #'(lambda (val num)
								 ...
								 (if (and (= num 2)
								   ...
								   (if (= num 3)
								       (let ((data (gethash (second val) vars)))
									 (if data
									     (setf (varinfo-gen-type data) 'frame->file))))))
...
(def-clm-fun 'sine-bank         #'(lambda (var x) (package-op '<sine-bank> var x :clm-real)))
...
(def-clm-fun 'mus-cosines       #'(lambda (var x) (gen-package-op '<mus-cosines> var x :clm-integer :mus-any)))
...
(def-clm-fun 'mus-formant-radius #'(lambda (var x) (gen-package-op '<mus-formant-radius> var x :clm-real :mus-any)))
...
(def-clm-fun 'phase-vocoder-outctr         #'(lambda (var x) (gen-package-op '<phase-vocoder-outctr> var x :clm-integer 'phase-vocoder)))
...
(def-clm-fun 'sound-frames        #'(lambda (var x) (package-op '<sound-frames> var x :clm-integer nil nil #'mark-as-string)))
...
(def-clm-fun 'mus-sound-frames        #'(lambda (var x) (package-op '<sound-frames> var x :clm-integer nil nil #'mark-as-string)))
...
(def-clm-fun 'waveshape         #'(lambda (var x) (gen-package-op '<waveshape> var x :clm-real 'waveshape)))
(def-clm-fun 'waveshape?        #'(lambda (var x) (gen-package-op '<waveshape?> var x :clm-boolean :mus-any)))
...
(def-clm-fun 'sum-of-cosines    #'(lambda (var x) (gen-package-op '<sum-of-cosines> var x :clm-real 'sum-of-cosines)))
(def-clm-fun 'sum-of-cosines?   #'(lambda (var x) (gen-package-op '<sum-of-cosines?> var x :clm-boolean :mus-any)))
(def-clm-fun 'sum-of-sines      #'(lambda (var x) (gen-package-op '<sum-of-sines> var x :clm-real 'sum-of-sines)))
(def-clm-fun 'sum-of-sines?     #'(lambda (var x) (gen-package-op '<sum-of-sines?> var x :clm-boolean :mus-any)))
...
(def-clm-fun 'file->frame       #'(lambda (var x)
				    (declare (ignore var))
				    (gen-package-op '<file2frame> nil x nil 'file->frame
						    #'(lambda (val num)
							(if (and (= num 2)
								 (listp val))
							    (let ((info (gethash (second val) vars)))
...
(def-clm-fun 'file->frame?      #'(lambda (var x) (gen-package-op '<file2sample?> var x :clm-boolean :mus-any)))
(def-clm-fun 'frame->file       #'(lambda (var x) (declare (ignore var)) (gen-package-op '<frame2file> nil x nil 'frame->file)))
(def-clm-fun 'frame->file?      #'(lambda (var x) (gen-package-op '<sample2file?> var x :clm-boolean :mus-any)))
...
(def-clm-fun 'frame?            #'(lambda (var x) (gen-package-op '<frame?> var x :clm-boolean :mus-any)))
(def-clm-fun 'mixer?            #'(lambda (var x) (gen-package-op '<mixer?> var x :clm-boolean :mus-any)))
(def-clm-fun 'frame+            #'(lambda (var x) (declare (ignore var)) (package-op '<frame+> nil x nil nil #'mark-as-frame) (fourth x)))
(def-clm-fun 'frame*            #'(lambda (var x) (declare (ignore var)) (package-op '<frame*> nil x nil nil #'mark-as-frame) (fourth x)))
(def-clm-fun 'mixer*            #'(lambda (var x) (declare (ignore var)) (package-op '<mixer*> nil x nil nil #'mark-as-mixer) (fourth x)))
(def-clm-fun 'mixer+            #'(lambda (var x) (declare (ignore var)) (package-op '<mixer+> nil x nil nil #'mark-as-mixer) (fourth x)))
(def-clm-fun 'mixer-scale       #'(lambda (var x) (declare (ignore var)) (gen-package-op '<mixer-scale> nil x nil 'mixer) (fourth x)))
(def-clm-fun 'frame-ref         #'(lambda (var x) (gen-package-op '<frame-ref> var x :clm-real 'frame #'mark-arg2-as-integer)))
(def-clm-fun 'mixer-ref         #'(lambda (var x) (gen-package-op '<mixer-ref> var x :clm-real 'mixer #'mark-arg2-as-integer)))
(def-clm-fun 'frame-set!        #'(lambda (var x)
				    (declare (ignore var))
				    (gen-package-op '<frame-set!> 0 x nil 'frame
						    #'(lambda (val num)
							(if (and (= num 0)
								 (listp val))
							    (let ((info (gethash (second val) vars)))
							      (when (and info
									 (not (varinfo-type info))
									 (not (varinfo-gen-type info)))
								(if *clm-debug* (format t "frame set ~A as frame~%" (second val)))
								(setf (varinfo-gen-type info) 'frame)))
							  (if (and (= num 1)
								   (listp val))
							      (let ((info (gethash (second val) vars)))
								(when (and info
									   (not (varinfo-type info)))
								  (if (varinfo-temp info)
								      (setf (varinfo-type info) :clm-integer)
								    (setf (varinfo-type info) :integer))))))))))
							    
(def-clm-fun 'mixer-set!        #'(lambda (var x)
				    (declare (ignore var))
				    (gen-package-op '<mixer-set!> 0 x nil 'mixer
						    #'(lambda (val num)
							(if (= num 0)
							    (if (listp val)
								(let ((info (gethash (second val) vars)))
								  (when (and info
									     (not (varinfo-type info))
									     (not (varinfo-gen-type info)))
								    (if *clm-debug* (format t "mixer set ~A as mixer~%" (second val)))
								    (setf (varinfo-gen-type info) 'mixer))))
							    (if (listp val)
								(let ((info (gethash (second val) vars)))
								  (when (and info
...
(def-clm-fun 'frame->frame      #'(lambda (var x) (declare (ignore var)) (gen-package-op '<frame2frame> nil x nil 'frame #'mark-as-frame2frame) (fourth x)))
...
(defun lc-num-ref (n &optional type)
  (if n
      (if (numberp n) 
	  (if (integerp n) n
	    #+(or excl openmcl) (coerce n 'short-float) 
	    #+(or cmu sbcl) (format nil "~F" (coerce n 'short-float))
					; CMU-CL appears to use "e" exponent despite ~F format directive
					; but C can understand that character, I think 
	    #+clisp (format nil "~F" n)
	    #-(or excl cmu sbcl openmcl clisp) n
	    )
...
(defmacro <random> (result arg)
  ...
  (checked-op1 result
	       #'(lambda (res var)
		   (declare (ignore res var))
		   (lc-set +setf+ result (format nil "(off_t)(mus_frandom((double)(~A)))" (lc-num-ref arg :integer)) :integer))
	       #'(lambda (res var)
		   (declare (ignore res var))
...
(defmacro <floor> (result arg &optional div-1)
  (if div-1
      (format *c-file* "  ~A~80,1T/* (floor ~A ~A) */~%"
	      (lc-set +setf+ result
		      (format nil "(off_t)floor((double)(~A) / (double)(~A))" (lc-num-ref arg) (lc-num-ref div-1)))
	      (clean-arg arg) (clean-arg div-1))
    (format *c-file* "  ~A~80,1T/* (floor ~A) */~%"
	    (lc-set +setf+ result
		    (format nil "(off_t)floor((double)(~A))" (lc-num-ref arg)))
	    (clean-arg arg)))
  nil)
...
(defmacro <ceiling> (result arg &optional div-1) 
  (if div-1
      (format *c-file* "  ~A~80,1T/* (ceiling ~A ~A) */~%"
	      (lc-set +setf+ result
		      (format nil "(off_t)ceil((double)(~A) / (double)(~A))" (lc-num-ref arg) (lc-num-ref div-1)))
	      (clean-arg arg) (clean-arg div-1))
    (format *c-file* "  ~A~80,1T/* (ceiling ~A) */~%"
	    (lc-set +setf+ result
		    (format nil "(off_t)ceil((double)(~A))" (lc-num-ref arg)))
	    (clean-arg arg)))
  nil)
...
(defmacro <round> (result arg &optional div-1)
  ...
  (if div-1
      (format *c-file* "  ~A~80,1T/* (round ~A ~A) */~%"
	      (lc-set +setf+ result
		      (format nil "(off_t)floor((double)(~A) / (double)(~A) + .5)" (lc-num-ref arg) (lc-num-ref div-1)))
	      (clean-arg arg) (clean-arg div-1))
    (format *c-file* "  ~A~80,1T/* (round ~A) */~%"
	    (lc-set +setf+ result
		    (format nil "(off_t)floor((double)((~A) + .5))" (lc-num-ref arg)))
	    (clean-arg arg)))
  (format *c-file* "  if ( (((~A) - (~A)) == .5) && (((off_t)(~A)) & 1) ) ~A;~%"
	  (lc-num-ref result)
	  (lc-num-ref arg)
	  (lc-num-ref result)
  ...
  (let* ((argl (mapcar #'lc-type args))
	  ...
	  (progn			;now for fUn WiTh fOrMaT
		...
		(if (or (char= nc #\A) (char= nc #\a))
			...
			(if (member arg '(:bignum :integer :clm-integer))
			    (progn (push #\l ns) (push #\l ns) (push #\d ns))
			  (if (eq arg :string)
			      (push #\s ns)
			    (if (member arg '(:float-array :double-array :integer-array))
				(push #\x ns)
			      (push #\f ns))))))
		  (if (member nc '(#\D #\d)) (progn (push #\l ns) (push #\l ns) (push #\d ns) (pop argl) (push :integer argtypes))
	...
	(setf (elt newstr i) (pop ns)))
      (format nil "\"~A~A\"~A~{~^~%        ~A~A~^,~}"
		...
		collect (if (eq typ :integer) "(off_t)"
...
(defun lisp-to-C-print (nl name fstr1 &rest args)
  ...
  (let* ((stdout (or (not fstr1) (stringp fstr1)))
	...
	(format nil "  mus_error(0, ~A);~%" named-cstr)
      (if (> (length cstr) 0)
	  (format nil "  {char *buf; buf=(char *)calloc(512, sizeof(char)); sprintf(buf, ~A); write(~A, buf, strlen(buf)); free(buf);}~%"
		  named-cstr
...
(defmacro <error> (&optional fstr &rest args) 
  (format *c-file* "  ~A  abort();~%" (apply #'lisp-to-C-print nil "error:" fstr args))
  nil)
...
(defmacro <clear-array> (data &optional len)
  (format *c-file* "  mus_clear_array(~A, ~A);~%"
...
;;; ---- SINE-BANK ----
(defmacro <sine-bank> (result s1 s2 &optional len)
  (format *c-file* "  ~A = mus_sine_bank(~A, ~A, ~A);~%"
	  (lc2 result) (lc-arr-ref s1) (lc-arr-ref s2) (if len (lc-num-ref len :integer) (array-size s1)))
  nil)
...
(defun run-type->class (type) ;first (second?) step in gen-unload process -- get type, dispatch to method
  (cond
   ...
   ((= type +sum-of-cosines+) (make-instance 'sum-of-cosines))
   ((= type +sum-of-sines+) (make-instance 'sum-of-sines))
   ...
   ((= type +sine-summation+) (make-instance 'sine-summation))
   ((= type +waveshape+) (make-instance 'waveshape))
   ((= type +polyshape+) (make-instance 'polyshape))
   ((= type +formant+) (make-instance 'formant))
   ((= type +sample2file+) (make-instance 'sample->file))
   ((= type +frame2file+) (make-instance 'frame->file))
   ((= type +file2sample+) (make-instance 'file->sample))
   ((= type +file2frame+) (make-instance 'file->frame))
...
(defmacro <sound-data-format> (res arg)
  (format *c-file* "  ~A = mus_sound_data_format((char *)(~A));~%" (lc2 res) (lc-ref arg :string))
  nil)
...
(defmacro <sound-frames> (res arg)
  (format *c-file* "  ~A = mus_sound_frames((char *)(~A));~%" (lc2 res) (lc-ref arg :string))
  nil)
...
(defmethod gen-load ((gen frame) i r datai datar)
  (setf (aref datai i) +frame+)
  (setf (aref datai (+ i 1)) r)
  (setf (aref datai (+ i 2)) (fr-chans gen))
  (loop for k from 0 below (fr-chans gen) do
    (setf (aref datar (+ r k)) (double (aref (fr-vals gen) k)))))

(defmethod gen-size ((gen frame)) (list 3 (fr-chans gen)))

(defmethod gen-unload ((gen frame) iadr radr datai datar)
  (declare (ignore radr))
  (let* ((i (aref datai (+ iadr 1)))
	 (r (aref datai (+ i 1))))
    (setf (fr-chans gen) (aref datai (+ i 2)))
    (setf (fr-vals gen) (make-double-array (fr-chans gen)))
    (loop for k from 0 below (fr-chans gen) do (setf (aref (fr-vals gen) k) (aref datar (+ r k))))
    gen))

(defmacro <frame?> (res arg)
  (format *c-file* "  ~A = mus_frame_p(~A);~%" (lc2 res) (lc2 arg))
  nil)

(defmethod gen-load ((gen mixer) i r datai datar)
  (setf (aref datai i) +mixer+)
  (setf (aref datai (+ i 1)) r)
  (setf (aref datai (+ i 2)) (mx-chans gen))
  (loop for k from 0 below (array-total-size (mx-vals gen)) do
    (setf (aref datar (+ r k)) (double (row-major-aref (mx-vals gen) k)))))

(defmethod gen-size ((gen mixer)) (list 3 (array-total-size (mx-vals gen))))

(defmethod gen-unload ((gen mixer) iadr radr datai datar)
  (declare (ignore radr))
  (let* ((i (aref datai (+ iadr 1)))
	 (r (aref datai (+ i 1))))
    (setf (mx-chans gen) (aref datai (+ i 2)))
    (setf (mx-vals gen) (make-double-array (list (mx-chans gen) (mx-chans gen))))
    (loop for k from 0 below (array-total-size (mx-vals gen)) do
      (setf (row-major-aref (mx-vals gen) k) (aref datar (+ r k))))
    gen))
  
(defmacro <mixer?> (res arg)
  (format *c-file* "  ~A = mus_mixer_p(~A);~%" (lc2 res) (lc2 arg))
  nil)

;; no make-frame|mixer in run currently, so 3rd arg is not optional

(defun is-number-var (n)
  (and (listp n)
       (eq (car n) :var)
       (member (varinfo-type (gethash (cadr n) vars)) '(:real :integer :clm-integer :clm-real))))

(defmacro <frame+> (fr1 fr2 outf)
  (if (or (numberp fr1) (is-number-var fr1))
      (format *c-file* "  mus_frame_offset(~A, ~A, ~A);~%" (lc2 fr2) (if (numberp fr1) (lc-num-ref fr1) (lc2 fr1)) (lc2 outf))
    (if (or (numberp fr2) (is-number-var fr2))
	(format *c-file* "  mus_frame_offset(~A, ~A, ~A);~%" (lc2 fr1) (if (numberp fr2) (lc-num-ref fr2) (lc2 fr2)) (lc2 outf))
      (format *c-file* "  mus_frame_add(~A, ~A, ~A);~%" (lc2 fr1) (lc2 fr2) (lc2 outf))))
  nil)

(defmacro <frame*> (fr1 fr2 outf)
  (if (or (numberp fr1) (is-number-var fr1))
      (format *c-file* "  mus_frame_scale(~A, ~A, ~A);~%" (lc2 fr2) (if (numberp fr1) (lc-num-ref fr1) (lc2 fr1)) (lc2 outf))
    (if (or (numberp fr2) (is-number-var fr2))
	(format *c-file* "  mus_frame_scale(~A, ~A, ~A);~%" (lc2 fr1) (if (numberp fr2) (lc-num-ref fr2) (lc2 fr2)) (lc2 outf))
      (format *c-file* "  mus_frame_multiply(~A, ~A, ~A);~%" (lc2 fr1) (lc2 fr2) (lc2 outf))))
  nil)

(defmacro <mixer*> (mx1 mx2 outm)
  (if (or (numberp mx1) (is-number-var mx1))
      (format *c-file* "  mus_mixer_scale(~A, ~A, ~A);~%" (lc2 mx2) (if (numberp mx1) (lc-num-ref mx1) (lc2 mx1)) (lc2 outm))
    (if (or (numberp mx2) (is-number-var mx2))
	(format *c-file* "  mus_mixer_scale(~A, ~A, ~A);~%" (lc2 mx1) (if (numberp mx2) (lc-num-ref mx2) (lc2 mx2)) (lc2 outm))
      (format *c-file* "  mus_mixer_multiply(~A, ~A, ~A);~%" (lc2 mx1) (lc2 mx2) (lc2 outm))))
  nil)

(defmacro <mixer+> (mx1 mx2 outm)
  (if (or (numberp mx1) (is-number-var mx1))
      (format *c-file* "  mus_mixer_offset(~A, ~A, ~A);~%" (lc2 mx2) (if (numberp mx1) (lc-num-ref mx1) (lc2 mx1)) (lc2 outm))
    (if (or (numberp mx2) (is-number-var mx2))
	(format *c-file* "  mus_mixer_offset(~A, ~A, ~A);~%" (lc2 mx1) (if (numberp mx2) (lc-num-ref mx2) (lc2 mx2)) (lc2 outm))
      (format *c-file* "  mus_mixer_add(~A, ~A, ~A);~%" (lc2 mx1) (lc2 mx2) (lc2 outm))))
  nil)

(defmacro <mixer-scale> (mx1 val outm)
  (format *c-file* "  mus_mixer_scale(~A, ~A, ~A);~%" (lc2 mx1) (lc-num-ref val) (lc2 outm))
  nil)

(defmacro <frame2frame> (mx1 fr2 outf)
  (format *c-file* "  mus_frame_to_frame(~A, ~A, ~A);~%" (lc2 mx1) (lc2 fr2) (lc2 outf))
  nil)
  
(defmacro <sample2frame> (fm samp outf) ;fm can be frame or mixer
  (format *c-file* "  mus_sample_to_frame(~A, ~A, ~A);~%" (lc2 fm) (lc-num-ref samp) (lc2 outf))
  nil)

(defmacro <frame2sample> (result fm inf)
  (format *c-file* "  ~A = mus_frame_to_sample(~A, ~A);~%" (lc2 result) (lc2 fm) (lc2 inf))
  nil)

(defmacro <frame2file> (stream pass inf)
  (format *c-file* "  mus_frame_to_file(~A, ~A, ~A);~%" (lc2 stream) (lc-num-ref pass :integer) (lc2 inf))
  nil)

(defmacro <frame-ref> (result fr chn)
  (format *c-file* "  ~A = mus_frame_ref(~A, ~A);~%" (lc2 result) (lc2 fr) (lc-num-ref chn :integer))
  nil)

(defmacro <frame-set!> (type fr chn val)
  ;; type = +incf+ +decf+ +setf+
  (format *c-file* "  mus_frame_set(~A, ~A, ~A);~%"
	  (lc2 fr)
	  (lc-num-ref chn :integer)
	  (if (= type +setf+)
	      (lc-num-ref val)
	    (format nil "mus_frame_ref(~A, ~A) ~A ~A"
		    (lc2 fr)
		    (lc-num-ref chn :integer)
		    (if (= type +incf+) "+" "-")
		    (lc-num-ref val))))
...
(defmacro <mixer-ref> (result mx in out)
  (format *c-file* "  ~A = mus_mixer_ref(~A, ~A, ~A);~%" (lc2 result) (lc2 mx) (lc-num-ref in :integer) (lc-num-ref out :integer))
  nil)

(defmacro <mixer-set!> (type mx in out val)
  (format *c-file* "  mus_mixer_set(~A, ~A, ~A, ~A);~%"
	  (lc2 mx)
	  (lc-num-ref in :integer)
	  (lc-num-ref out :integer)
	  (if (= type +setf+)
	      (lc-num-ref val)
	    (format nil "mus_mixer_ref(~A, ~A, ~A) ~A ~A"
		    (lc2 mx)
		    (lc-num-ref in :integer)
		    (lc-num-ref out :integer)
		    (if (= type +incf+) "+" "-")
...
(defmethod gen-reflect ((gen frame) key val)
  (let ((name (lc key))
	(iadr (varinfo-iloc val)))
    (format *c-file* "  if (mus_frame_p(~A))~%    ~
                          {~%     ~
                            int i;~%      ~
                            for (i = 0; i < mus_length(~A); i++)~%        ~
                              ((double *)(clm_double + CLM_RLOC(~A)))[i] = mus_frame_ref(~A, i);~%    ~
                          }~%"
	    name
	    name
	    iadr name))
...
(defmethod gen-make ((gen frame) stream result iloc indent ctr)
  (declare (ignore ctr))
  (format stream "~Aif (CLM_FRAME_P(~A))~%~A  ~
                      ~A = clm_add_gen_to_genbag(all_gens, ~%~A    ~
                         mus_make_frame_with_data(~%~A      ~
                           clm_int[CLM_ILOC(~A) + 2],/* chans */~%~A      ~
                           (double *)(clm_double + CLM_RLOC(~A))));/* data */~%"
	  indent iloc indent
	  result indent indent
	  iloc indent
	  iloc))

(defmethod gen-reflect ((gen mixer) key val)
  (let ((name (lc key))
	(iadr (varinfo-iloc val)))
    (format *c-file* "  if (mus_mixer_p(~A))~%    ~
                          {~%     ~
                            int i, j;~%      ~
                            for (i = 0; i < mus_length(~A); i++)~%        ~
                              for (j = 0; j < mus_length(~A); j++)~%          ~
                                ((double *)(clm_double + CLM_RLOC(~A)))[i * mus_length(~A) + j] = mus_mixer_ref(~A, i, j);~%    ~
                          }~%"
	    name
	    name
	    name
...
(defmethod gen-make ((gen mixer) stream result iloc indent ctr)
  (declare (ignore ctr))
  (format stream "~Aif(CLM_MIXER_P(~A))~%~A  ~
                      ~A = clm_add_gen_to_genbag(all_gens, ~%~A    ~
                         mus_make_mixer_with_data(~%~A      ~
                           clm_int[CLM_ILOC(~A) + 2],/* chans */~%~A      ~
                           (double *)(clm_double + CLM_RLOC(~A))));/* data */~%"
	  indent iloc indent
	  result indent indent
...
(defmacro <oscil?> (res arg)
  (format *c-file* "  ~A = mus_oscil_p(~A);~%" (lc2 res) (lc2 arg))
  nil)
...
(defmacro <oscil> (result s &optional fm pm)
  (let* ((pm-in-use (and pm (or (not (constantp pm)) (not (zerop pm)))))
	 ...
	 (resname (lc2 result)))
    (when (> *safety* 0)
      (format *c-file* "  if (!(mus_oscil_p(~A))) {mus_error(0, \"oscil ~(~A~) is %s\", mus_describe(~A)); goto RUN_ALL_DONE;}~%"
	...
	(format *c-file* "  ~A = mus_oscil(~A, ~A, ~A);~80,1T/* (oscil ~A ~A ~A) */~%"
      ...
      (if fm-in-use
	  (format *c-file* "  ~A = mus_oscil_1(~A, ~A);~80,1T/* (oscil ~A ~A) */~%"
		  resname cname (lc-num-ref fm) (clean-arg s) (clean-arg fm))
	(format *c-file* "  ~A = mus_oscil_0(~A);~80,1T/* (oscil ~A) */~%"
...
(defmethod gen-reflect ((gen oscil) key val)
  (let ((name (lc key))
	(iadr (varinfo-iloc val)))
    (format *c-file* "  if (mus_oscil_p(~A))~%    ~
                          {~%     ~
...
(defmacro <square-wave?> (res arg)
  (format *c-file* "  ~A = mus_square_wave_p(~A);~%" (lc2 res) (lc2 arg))
  nil)

(defmacro <pulse-train?> (res arg)
  (format *c-file* "  ~A = mus_pulse_train_p(~A);~%" (lc2 res) (lc2 arg))
  nil)

(defmacro <triangle-wave?> (res arg)
  (format *c-file* "  ~A = mus_triangle_wave_p(~A);~%" (lc2 res) (lc2 arg))
  nil)

(defmacro <sawtooth-wave?> (res arg)
  (format *c-file* "  ~A = mus_sawtooth_wave_p(~A);~%" (lc2 res) (lc2 arg))
  nil)

(defmacro <square-wave> (result s &optional fm)
  (when (> *safety* 0)
    (format *c-file* "  if (!(mus_square_wave_p(~A))) {mus_error(0, \"square-wave ~(~A~) is %s\", mus_describe(~A)); goto RUN_ALL_DONE;}~%"
...
(defmacro <pulse-train> (result s &optional fm)
  (when (> *safety* 0)
    (format *c-file* "  if (!(mus_pulse_train_p(~A))) {mus_error(0, \"pulse-train ~(~A~) is %s\", mus_describe(~A)); goto RUN_ALL_DONE;}~%"
	    (lc2 s) (clean-arg s) (lc2 s)))
...
(defmacro <triangle-wave> (result s &optional fm)
  (when (> *safety* 0)
    (format *c-file* "  if (!(mus_triangle_wave_p(~A))) {mus_error(0, \"triangle-wave ~(~A~) is %s\", mus_describe(~A)); goto RUN_ALL_DONE;}~%"
	    (lc2 s) (clean-arg s) (lc2 s)))
...
(defmacro <sawtooth-wave> (result s &optional fm)
  (when (> *safety* 0)
    (format *c-file* "  if (!(mus_sawtooth_wave_p(~A))) {mus_error(0, \"sawtooth-wave ~(~A~) is %s\", mus_describe(~A)); goto RUN_ALL_DONE;}~%"
	    (lc2 s) (clean-arg s) (lc2 s)))
...
(defmethod gen-reflect ((gen triangle-wave) key val)
  (let ((name (lc key))
	(iadr (varinfo-iloc val)))
    (format *c-file* "  if (mus_triangle_wave_p(~A))~%    ~
                          {~%     ~
                           ~A = mus_hz_to_radians(mus_frequency(~A));~%     ~
                           ~A = mus_phase(~A);~%     ~
                           ~A = mus_scaler(~A);~%    ~
...
(defmethod gen-reflect ((gen square-wave) key val)
  (let ((name (lc key))
	(iadr (varinfo-iloc val)))
    (format *c-file* "  if (mus_square_wave_p(~A))~%    ~
                          {~%     ~
                           ~A = mus_hz_to_radians(mus_frequency(~A));~%     ~
                           ~A = mus_phase(~A);~%     ~
                           ~A = mus_scaler(~A);~%    ~
...
(defmethod gen-reflect ((gen sawtooth-wave) key val)
  (let ((name (lc key))
	(iadr (varinfo-iloc val)))
    (format *c-file* "  if (mus_sawtooth_wave_p(~A))~%    ~
                          {~%     ~
                           ~A = mus_hz_to_radians(mus_frequency(~A));~%     ~
                           ~A = mus_phase(~A);~%     ~
                           ~A = mus_scaler(~A);~%    ~
...
(defmethod gen-reflect ((gen pulse-train) key val)
  (let ((name (lc key))
	(iadr (varinfo-iloc val)))
    (format *c-file* "  if (mus_pulse_train_p(~A))~%    ~
                          {~%     ~
                           ~A = mus_hz_to_radians(mus_frequency(~A));~%     ~
                           ~A = mus_phase(~A);~%     ~
                           ~A = mus_scaler(~A);~%    ~
...
(defmethod gen-load ((s sum-of-cosines) i r datai datar)
  (setf (aref datai i) +sum-of-cosines+)
  (setf (aref datai (+ i 1)) r)
  (setf (aref datai (+ i 2)) (cosp-cosines s))
  (setf (aref datar r) (double (cosp-freq s)))
  (setf (aref datar (+ r 1)) (double (cosp-phase s)))
  (setf (aref datar (+ r 2)) (double (cosp-scaler s))))

(defmethod gen-unload ((gen sum-of-cosines) iadr radr datai datar)
  (declare (ignore radr))
  (let* ((i (aref datai (+ iadr 1)))
	 (r (aref datai (+ i 1))))
    (setf (cosp-cosines gen) (aref datai (+ i 2)))
    (setf (cosp-freq gen) (aref datar r))
    (setf (cosp-phase gen) (aref datar (+ r 1)))
    (setf (cosp-scaler gen) (aref datar (+ r 2)))
    gen))

(defmethod gen-reflect ((gen sum-of-cosines) key val)
  (let ((name (lc key))
	(iadr (varinfo-iloc val)))
    (format *c-file* "  if (mus_sum_of_cosines_p(~A))~%    {~%     ~
                           ~A = mus_cosines(~A);~%     ~
                           ~A = mus_hz_to_radians(mus_frequency(~A));~%     ~
                           ~A = mus_phase(~A);~%     ~
                           ~A = mus_scaler(~A);~%    }~%"
...
(defmethod gen-size ((gen sum-of-cosines)) (list 3 3))

(defmethod gen-make ((gen sum-of-cosines) stream result iloc indent ctr)
  (declare (ignore ctr))
  (format stream "~Aif (CLM_SUM_OF_COSINES_P(~A))~%~A  ~
                      ~A = clm_add_gen_to_genbag(all_gens, ~%~A    ~
                         mus_make_sum_of_cosines(~%~A      ~
                           clm_int[CLM_ILOC(~A) + 2],~%~A      ~
                           mus_radians_to_hz(clm_double[CLM_RLOC(~A)]),~%~A      ~
                           clm_double[CLM_RLOC(~A) + 1]));~%"
	  indent iloc indent result indent indent iloc indent iloc indent iloc))

(defmacro <sum-of-cosines> (result s &optional fm)
  (when (> *safety* 0)
    (format *c-file* "  if (!(mus_sum_of_cosines_p(~A))) {mus_error(0, \"sum-of-cosines ~(~A~) is %s\", mus_describe(~A)); goto RUN_ALL_DONE;}~%"
	    (lc2 s) (clean-arg s) (lc2 s)))
  (format *c-file* "  ~A = mus_sum_of_cosines(~A, ~A);~80,1T/* (sum-of-cosines ~A~A) */~%"
	  (lc2 result)
	  (lc2 s)
	  (if fm (lc-num-ref fm) "0.0")
	  (clean-arg s)
...
(defmacro <sum-of-cosines?> (res arg)
  (format *c-file* "  ~A = mus_sum_of_cosines_p(~A);~%" (lc2 res) (lc2 arg))
  nil)


;;; ---- SUM-OF-SINES ----

(defmethod gen-load ((s sum-of-sines) i r datai datar)
  (setf (aref datai i) +sum-of-sines+)
  (setf (aref datai (+ i 1)) r)
  (setf (aref datai (+ i 2)) (cosp-cosines s))
  (setf (aref datar r) (double (cosp-freq s)))
  (setf (aref datar (+ r 1)) (double (cosp-phase s)))
...
(defmethod gen-unload ((gen sum-of-sines) iadr radr datai datar)
  (declare (ignore radr))
  (let* ((i (aref datai (+ iadr 1)))
	 (r (aref datai (+ i 1))))
    (setf (cosp-cosines gen) (aref datai (+ i 2)))
...
(defmethod gen-reflect ((gen sum-of-sines) key val)
  (let ((name (lc key))
	(iadr (varinfo-iloc val)))
    (format *c-file* "  if (mus_sum_of_sines_p(~A))~%    {~%     ~
                           ~A = mus_cosines(~A);~%     ~
                           ~A = mus_hz_to_radians(mus_frequency(~A));~%     ~
                           ~A = mus_phase(~A);~%     ~
                           ~A = mus_scaler(~A);~%    }~%"
...
(defmethod gen-size ((gen sum-of-sines)) (list 3 3))

(defmethod gen-make ((gen sum-of-sines) stream result iloc indent ctr)
  (declare (ignore ctr))
  (format stream "~Aif (CLM_SUM_OF_SINES_P(~A))~%~A  ~
                      ~A = clm_add_gen_to_genbag(all_gens, ~%~A    ~
                         mus_make_sum_of_sines(~%~A      ~
                           clm_int[CLM_ILOC(~A) + 2],~%~A      ~
                           mus_radians_to_hz(clm_double[CLM_RLOC(~A)]),~%~A      ~
                           clm_double[CLM_RLOC(~A) + 1]));~%"
	  indent iloc indent result indent indent iloc indent iloc indent iloc))

(defmacro <sum-of-sines> (result s &optional fm)
  (when (> *safety* 0)
    (format *c-file* "  if (!(mus_sum_of_sines_p(~A))) {mus_error(0, \"sum-of-sines ~(~A~) is %s\", mus_describe(~A)); goto RUN_ALL_DONE;}~%"
	    (lc2 s) (clean-arg s) (lc2 s)))
  (format *c-file* "  ~A = mus_sum_of_sines(~A, ~A);~80,1T/* (sum-of-sines ~A~A) */~%"
	  (lc2 result)
	  (lc2 s)
	  (if fm (lc-num-ref fm) "0.0")
	  (clean-arg s)
...
(defmacro <sum-of-sines?> (res arg)
  (format *c-file* "  ~A = mus_sum_of_sines_p(~A);~%" (lc2 res) (lc2 arg))
  nil)
...
(defmacro <ssb-am?> (res arg)
  (format *c-file* "  ~A = mus_ssb_am_p(~A);~%" (lc2 res) (lc2 arg))
  nil)

(defmacro <ssb-am> (result s insig &optional fm)
  (when (> *safety* 0)
    (format *c-file* "  if (!(mus_ssb_am_p(~A))) {mus_error(0, \"ssb-am ~(~A~) is %s\", mus_describe(~A)); goto RUN_ALL_DONE;}~%"
	    (lc2 s) (clean-arg s) (lc2 s)))
  (let* ((fm-in-use (and fm (or (not (constantp fm)) (not (zerop fm)))))
	...
	(format *c-file* "  ~A = mus_ssb_am(~A, ~A, ~A);~80,1T/* (ssb-am ~A ~A ~A) */~%"
      ...
      (format *c-file* "  ~A = mus_ssb_am_1(~A, ~A);~80,1T/* (ssb-am ~A ~A) */~%"
...
(defmacro <rand?> (res arg)
  (format *c-file* "  ~A = mus_rand_p(~A);~%" (lc2 res) (lc2 arg))
  nil)

(defmacro <rand-interp?> (res arg)
  (format *c-file* "  ~A = mus_rand_interp_p(~A);~%" (lc2 res) (lc2 arg))
  nil)

(defmacro <rand> (result s &optional fm)
  (when (> *safety* 0)
    (format *c-file* "  if (!(mus_rand_p(~A))) {mus_error(0, \"rand ~(~A~) is %s\", mus_describe(~A)); goto RUN_ALL_DONE;}~%"
	    (lc2 s) (clean-arg s) (lc2 s)))
...
(defmacro <rand-interp> (result s &optional fm)
  (when (> *safety* 0)
    (format *c-file* "  if (!(mus_rand_interp_p(~A))) {mus_error(0, \"rand-interp ~(~A~) is %s\", mus_describe(~A)); goto RUN_ALL_DONE;}~%"
	    (lc2 s) (clean-arg s) (lc2 s)))
...
(defmethod gen-reflect ((gen rand-interp) key val)
  (let ((name (lc key))
	(iadr (varinfo-iloc val)))
    (format *c-file* "  if (mus_rand_interp_p(~A))~%    ~
                          {~%     ~
                           ~A = mus_frequency(~A);~%     ~
                           ~A = mus_scaler(~A);~%    ~
                          }~%"
...
(defmacro <table-lookup> (result s &optional fm)
  (when (> *safety* 0)
    (format *c-file* "  if (!(mus_table_lookup_p(~A))) {mus_error(0, \"table-lookup ~(~A~) is %s\", mus_describe(~A)); goto RUN_ALL_DONE;}~%"
	    (lc2 s) (clean-arg s) (lc2 s)))
  (if fm
      (format *c-file* "  ~A = mus_table_lookup(~A, ~A);~%" (lc2 result) (lc2 s) (lc-num-ref fm))
    (format *c-file* "  ~A = mus_table_lookup_1(~A);~%" (lc2 result) (lc2 s)))
  nil)

(defmacro <table-lookup?> (res arg)
  (format *c-file* "  ~A = mus_table_lookup_p(~A);~%" (lc2 res) (lc2 arg))
  nil)
...
(defmethod gen-reflect ((gen table-lookup) key val)
  (let ((name (lc key))
	(iadr (varinfo-iloc val)))
    (format *c-file* "  if (mus_table_lookup_p(~A))~%    ~
                          {~%     ~
                           ~A = mus_frequency(~A);~%     ~
                           ~A = mus_phase(~A);~%    ~
                          }~%"
...
(defmacro <file2array> (filename chan start frames arr)
...
(defmacro <one-pole> (result s arg)
  (when (> *safety* 0)
    (format *c-file* "  if (!(mus_one_pole_p(~A))) {mus_error(0, \"one-pole ~(~A~) is %s\", mus_describe(~A)); goto RUN_ALL_DONE;}~%"
	    (lc2 s) (clean-arg s) (lc2 s)))
...
(defmacro <one-pole?> (res arg)
  (format *c-file* "  ~A = mus_one_pole_p(~A);~%" (lc2 res) (lc2 arg))
  nil)

(defmethod gen-reflect ((gen one-pole) key val)
  (let ((name (lc key))
	(iadr (varinfo-iloc val)))
    (format *c-file* "  if (mus_one_pole_p(~A))~%    ~
                          {~%     ~
                           ~A = mus_xcoeff(~A, 0);~%     ~
                           ~A = mus_ycoeff(~A, 1);~%    ~
                          }~%"
...
(defmacro <one-zero> (result s arg)
  (when (> *safety* 0)
    (format *c-file* "  if (!(mus_one_zero_p(~A))) {mus_error(0, \"one-zero ~(~A~) is %s\", mus_describe(~A)); goto RUN_ALL_DONE;}~%"
	    (lc2 s) (clean-arg s) (lc2 s)))
...
(defmacro <one-zero?> (res arg)
  (format *c-file* "  ~A = mus_one_zero_p(~A);~%" (lc2 res) (lc2 arg))
  nil)

(defmethod gen-reflect ((gen one-zero) key val)
  (let ((name (lc key))
	(iadr (varinfo-iloc val)))
    (format *c-file* "  if (mus_one_zero_p(~A))~%    ~
                          {~%     ~
                           ~A = mus_xcoeff(~A, 0);~%     ~
                           ~A = mus_xcoeff(~A, 1);~%    ~
                          }~%"
...
(defmacro <two-pole> (result s arg)
  (when (> *safety* 0)
    (format *c-file* "  if (!(mus_two_pole_p(~A))) {mus_error(0, \"two-pole ~(~A~) is %s\", mus_describe(~A)); goto RUN_ALL_DONE;}~%"
	    (lc2 s) (clean-arg s) (lc2 s)))
...
(defmacro <two-pole?> (res arg)
  (format *c-file* "  ~A = mus_two_pole_p(~A);~%" (lc2 res) (lc2 arg))
  nil)

(defmethod gen-reflect ((gen two-pole) key val)
  (let ((name (lc key))
	(iadr (varinfo-iloc val)))
    (format *c-file* "  if (mus_two_pole_p(~A))~%    ~
                          {~%     ~
                           ~A = mus_xcoeff(~A, 0);~%     ~
                           ~A = mus_ycoeff(~A, 1);~%     ~
                           ~A = mus_ycoeff(~A, 2);~%    ~
...
(defmacro <two-zero> (result s arg)
  (when (> *safety* 0)
    (format *c-file* "  if (!(mus_two_zero_p(~A))) {mus_error(0, \"two-zero ~(~A~) is %s\", mus_describe(~A)); goto RUN_ALL_DONE;}~%"
	    (lc2 s) (clean-arg s) (lc2 s)))
...
(defmacro <two-zero?> (res arg)
  (format *c-file* "  ~A = mus_two_zero_p(~A);~%" (lc2 res) (lc2 arg))
  nil)

(defmethod gen-reflect ((gen two-zero) key val)
  (let ((name (lc key))
	(iadr (varinfo-iloc val)))
    (format *c-file* "  if (mus_two_zero_p(~A))~%    ~
                          {~%     ~
                           ~A = mus_xcoeff(~A, 0);~%     ~
                           ~A = mus_xcoeff(~A, 1);~%     ~
                           ~A = mus_xcoeff(~A, 2);~%    ~
...
(defmethod gen-load ((s formant) i r datai datar)
  ...
  (setf (aref datai (+ i 1)) r)
  (setf (aref datar r) (double (gain s)))
  (setf (aref datar (+ r 1)) (double (radius s)))
  (setf (aref datar (+ r 2)) (double (frequency s))))
...
(defmethod gen-unload ((s formant) iadr radr datai datar)
  ...
  (let* ((i (aref datai (+ iadr 1)))
    ...
    (setf (mus-formant-radius s) (aref datar (+ r 1)))
    (setf (mus-frequency s) (aref datar (+ r 2)))
    s))

(defmacro <formant> (result s arg)
  (when (> *safety* 0)
    (format *c-file* "  if (!(mus_formant_p(~A))) {mus_error(0, \"formant ~(~A~) is %s\", mus_describe(~A)); goto RUN_ALL_DONE;}~%"
	    (lc2 s) (clean-arg s) (lc2 s)))
...
(defmacro <formant?> (res arg)
  (format *c-file* "  ~A = mus_formant_p(~A);~%" (lc2 res) (lc2 arg))
  nil)

(defmethod gen-reflect ((gen formant) key val)
  (let ((name (lc key))
	(iadr (varinfo-iloc val)))
    (format *c-file* "  if (mus_formant_p(~A))~%    ~
                          {~%     ~
                           ~A = mus_scaler(~A);~%     ~
                           ~A = mus_phase(~A);~%     ~
                           ~A = mus_frequency(~A);~%    ~
...
(defmethod gen-load ((w waveshape) i r datai datar)
  (setf (aref datai i) +waveshape+)
  (setf (aref datai (+ i 1)) r)
...
(defmethod gen-size ((s waveshape))
  (list (+ 3 6)
	(+ 2 (length (ws-wave s)))))

(defmethod gen-unload ((s waveshape) iadr radr datai datar)
  (declare (ignore radr))
  (let* ((i (aref datai (+ iadr 1)))
	 (r (aref datai (+ i 1))))
...
(defmacro <waveshape?> (res arg)
  (format *c-file* "  ~A = mus_waveshape_p(~A);~%" (lc2 res) (lc2 arg))
  nil)

(defmacro <waveshape> (result s &optional (index 1.0) fm)
  (when (> *safety* 0)
    (format *c-file* "  if (!(mus_waveshape_p(~A))) {mus_error(0, \"waveshape ~(~A~) is %s\", mus_describe(~A)); goto RUN_ALL_DONE;}~%"
	    (lc2 s) (clean-arg s) (lc2 s)))
  (if fm
      (format *c-file* "  ~A = mus_waveshape(~A, ~A, ~A);~80,1T/* (waveshape ~A ~A ~A) */~%"
	      (lc2 result) (lc2 s) (lc-num-ref index) (lc-num-ref fm)
	      (clean-arg s) (clean-arg index) (clean-arg fm))
    (format *c-file* "  ~A = mus_waveshape_1(~A, ~A);~80,1T/* (waveshape ~A ~A) */~%"
	    (lc2 result) (lc2 s) (lc-num-ref index)
	    (clean-arg s) (clean-arg index)))
  nil)

(defmethod gen-reflect ((gen waveshape) key val)
  (let ((name (lc key))
	(iadr (varinfo-iloc val)))
    (format *c-file* "  if (mus_waveshape_p(~A))~%    ~
                          {~%     ~
                           ~A = mus_hz_to_radians(mus_frequency(~A));~%     ~
                           ~A = mus_phase(~A);~%    ~
                          }~%"
...
(defmethod gen-make ((gen waveshape) stream result iloc indent ctr)
  (declare (ignore ctr))
  (format stream "~Aif (CLM_WAVESHAPE_P(~A))~%~A  ~
                      ~A = clm_add_gen_to_genbag(all_gens, ~%~A    ~
                         mus_make_waveshape(~%~A      ~
                           mus_radians_to_hz(clm_double[CLM_RLOC(~A)]),~%~A      ~
                           clm_double[CLM_RLOC(~A) + 1],~%~A      ~
                           (double *)(clm_double + CLM_RLOC(~A) + 2),~%~A      ~
                           clm_int[CLM_ILOC(~A) + 2]));~%"
...
(defmacro <polyshape?> (res arg)
  (format *c-file* "  ~A = mus_polyshape_p(~A);~%" (lc2 res) (lc2 arg))
  nil)

(defmacro <polyshape> (result s &optional (index 1.0) fm)
  (when (> *safety* 0)
    (format *c-file* "  if (!(mus_polyshape_p(~A))) {mus_error(0, \"polyshape ~(~A~) is %s\", mus_describe(~A)); goto RUN_ALL_DONE;}~%"
	    (lc2 s) (clean-arg s) (lc2 s)))
  (if fm
      (format *c-file* "  ~A = mus_polyshape(~A, ~A, ~A);~80,1T/* (polyshape ~A ~A ~A) */~%"
	      ...
	      (clean-arg s) (clean-arg index) (clean-arg fm))
    (if index
	(format *c-file* "  ~A = mus_polyshape_1(~A, ~A);~80,1T/* (polyshape ~A ~A) */~%"
		(lc2 result) (lc2 s) (lc-num-ref index)
		(clean-arg s) (clean-arg index))
      (format *c-file* "  ~A = mus_polyshape_0(~A);~80,1T/* (polyshape ~A) */~%"
		(lc2 result) (lc2 s)
		(clean-arg s))))
  nil)
...
(defmethod gen-reflect ((gen polyshape) key val)
  (let ((name (lc key))
	(iadr (varinfo-iloc val)))
    (format *c-file* "  if (mus_polyshape_p(~A))~%    ~
                          {~%     ~
                           ~A = mus_hz_to_radians(mus_frequency(~A));~%     ~
                           ~A = mus_phase(~A);~%    ~
                          }~%"
...
;;;
;;; 0type 1:datar 2:n
;;; 0:freq 1:phase 2:a 3:b

(defmethod gen-load ((w sine-summation) i r datai datar)
  (setf (aref datai i) +sine-summation+)
  (setf (aref datai (+ i 1)) r)
  (setf (aref datai (+ i 2)) (or (sss-n w) 0))
  (setf (aref datar r) (double (sss-freq w)))
  (setf (aref datar (+ r 1)) (double (sss-phase w)))
  (setf (aref datar (+ r 2)) (double (sss-a w)))
  (setf (aref datar (+ r 3)) (double (sss-b w))))

(defmethod gen-size ((gen sine-summation)) (list 3 4))

(defmethod gen-unload ((gen sine-summation) iadr radr datai datar)
  (declare (ignore radr))
  (let* ((i (aref datai (+ iadr 1)))
	 (r (aref datai (+ i 1))))
    (setf (sss-freq gen) (aref datar r))
    (setf (sss-phase gen) (aref datar (+ r 1)))
    (setf (sss-a gen) (aref datar (+ r 2)))
    (setf (sss-b gen) (aref datar (+ r 3)))
    (setf (sss-n gen) (aref datai (+ i 2)))
    gen))
  
(defmacro <sine-summation?> (res arg)
  (format *c-file* "  ~A = mus_sine_summation_p(~A);~%" (lc2 res) (lc2 arg))
  nil)

(defmacro <sine-summation> (result s &optional fm)
  (when (> *safety* 0)
    (format *c-file* "  if (!(mus_sine_summation_p(~A))) {mus_error(0, \"sine-summation ~(~A~) is %s\", mus_describe(~A)); goto RUN_ALL_DONE;}~%"
	    (lc2 s) (clean-arg s) (lc2 s)))
  (format *c-file* "  ~A = mus_sine_summation(~A, ~A);~80,1T/* (sine-summation ~A~A) */~%"
	  (lc2 result) (lc2 s)
	  (if fm (lc-num-ref fm) "0.0")
	  (clean-arg s)
	  (if fm (format nil " ~A" (clean-arg fm)) ""))
...
(defmethod gen-reflect ((gen sine-summation) key val)
  (let ((name (lc key))
	(iadr (varinfo-iloc val)))
    (format *c-file* "  if (mus_sine_summation_p(~A))~%    ~
                          {~%     ~
                           ~A = mus_hz_to_radians(mus_frequency(~A));~%     ~
                           ~A = mus_phase(~A);~%     ~
                           ~A = mus_cosines(~A); /* n */~%     ~
...
(defmethod gen-make ((gen sine-summation) stream result iloc indent ctr)
  (declare (ignore ctr))
  (format stream "~Aif (CLM_SINE_SUMMATION_P(~A))~%~A  ~
                      ~A = clm_add_gen_to_genbag(all_gens, ~%~A    ~
                         mus_make_sine_summation(~%~A      ~
                           mus_radians_to_hz(clm_double[CLM_RLOC(~A)]),~%~A      ~
                           clm_double[CLM_RLOC(~A) + 1],~%~A      ~
                           clm_int[CLM_ILOC(~A) + 2],~%~A      ~
                           clm_double[CLM_RLOC(~A) + 2],~%~A      ~
                           clm_double[CLM_RLOC(~A) + 3]));~%"
	  indent iloc indent result indent indent iloc indent iloc indent iloc indent iloc indent iloc))
...
(defmacro <asymmetric-fm?> (res arg)
  (format *c-file* "  ~A = mus_asymmetric_fm_p(~A);~%" (lc2 res) (lc2 arg))
  nil)

(defmacro <asymmetric-fm> (result s index &optional fm)
  (when (> *safety* 0)
    (format *c-file* "  if (!(mus_asymmetric_fm_p(~A))) {mus_error(0, \"asymmetric-fm ~(~A~) is %s\", mus_describe(~A)); goto RUN_ALL_DONE;}~%"
	    (lc2 s) (clean-arg s) (lc2 s)))
...
(defmethod gen-reflect ((gen asymmetric-fm) key val)
  (let ((name (lc key))
	(iadr (varinfo-iloc val)))
    (format *c-file* "  if (mus_asymmetric_fm_p(~A))~%    ~
                          {~%     ~
                           ~A = mus_hz_to_radians(mus_frequency(~A));~%     ~
                           ~A = mus_phase(~A);~%     ~
                           ~A = mus_scaler(~A); /* a */~%     ~
...
(defmacro <out-any> (result pass val &optional (chan 0) stream)
  ...
  (when (> *safety* 0)
    (format *c-file* "  if (!(mus_output_p(~A))) {mus_error(0, \"output stream ~(~A~) is %s\", mus_describe(~A)); goto RUN_ALL_DONE;}~%"
...
(defmacro <mus-output?> (res gen)
  (if (generator? gen)
      (format *c-file* "  ~A = mus_output_p(~A);~%" (lc2 res) (lc2 gen))
...
(defmethod gen-load ((rd frame->file) i r datai datar)
  (setf (aref datai i) +frame2file+)
  (gen-load (mus-file-name rd) (+ i 1) r datai datar))

(defmethod gen-size ((rd frame->file))
  (list (+ 1 2 (ceiling (1+ (length (mus-file-name rd))) 4)) 0))

(defmethod gen-unload ((rd frame->file) iadr radr datai datar)
  (declare (ignore iadr radr datai datar))
  nil)
...
(defmethod gen-reflect ((gen frame->file) key val)
  (declare (ignore key val))
  nil)

(defmethod gen-make ((gen sample->file) stream result iloc indent ctr)
  (declare (ignore ctr))
  (format stream "~Aif ((CLM_SAMPLE2FILE_P(~A)) || (CLM_FRAME2FILE_P(~A)))~%~A  ~
                      ~A = clm_add_gen_to_genbag(all_gens, ~%~A    ~
...
(defmethod gen-make ((gen frame->file) stream result iloc indent ctr)
  (declare (ignore ctr))
  (if (equal result "_OUTPUT_")
	...
	(format stream "~A_REVERB_ = clm_reverb();~%" indent)
      (format stream "~Aif ((CLM_FRAME2FILE_P(~A)) || (CLM_SAMPLE2FILE_P(~A)))~%~A  ~
                        ~A = clm_add_gen_to_genbag(all_gens, ~%~A    ~
                           mus_make_frame_to_file_with_comment((char *)(clm_int + CLM_ILOC(~A) + 1 + 2), 1, MUS_BSHORT, MUS_NEXT, NULL));~%"
	  indent iloc iloc indent
	  result indent
	  iloc))))
...
(defmethod gen-load ((rd file->frame) i r datai datar)
  (setf (aref datai i) +file2frame+)
  (setf (aref datai (+ i 1)) (f2f-size rd))
  (gen-load (mus-file-name rd) (+ i 2) r datai datar))

(defmethod gen-size ((rd file->frame))
  (list (+ 2 2 (ceiling (1+ (length (mus-file-name rd))) 4)) 0))

(defmethod gen-unload ((rd file->frame) iadr radr datai datar)
  (declare (ignore iadr radr datai datar))
  nil)

(defmacro <in-any> (result samp chan i-stream)
  (when (> *safety* 0)
    (format *c-file* "  if (!(mus_input_p(~A))) {mus_error(0, \"input stream ~(~A~) is %s\", mus_describe(~A)); goto RUN_ALL_DONE;}~%"
...
(defmacro <file2sample?> (res arg)
  (format *c-file* "  ~A = mus_file_to_sample_p(~A);~%" (lc2 res) (lc2 arg))
  nil)
...
(defmacro <mus-input?> (res gen)
  (if (generator? gen)
      (format *c-file* "  ~A = mus_input_p(~A);~%" (lc2 res) (lc2 gen))
    (format *c-file* "  ~A = 0;~%" (lc2 res)))
  nil)

(defmacro <file2frame> (i-stream samp outf)
  (format *c-file* "  mus_file_to_frame(~A, ~A, ~A);~%"
...
(defmethod gen-reflect ((gen file->frame) key val)
  (declare (ignore key val))
  nil)

(defmethod gen-make ((gen file->sample) stream result iloc indent ctr)
  (declare (ignore ctr))
  (format stream "~Aif ((CLM_FILE2SAMPLE_P(~A)) || (CLM_FILE2FRAME_P(~A)))~%~A  ~
                      ~A = clm_add_gen_to_genbag(all_gens, ~%~A    ~
...
(defmethod gen-make ((gen file->frame) stream result iloc indent ctr)
  (declare (ignore ctr))
  (format stream "~Aif ((CLM_FILE2FRAME_P(~A)) || (CLM_FILE2SAMPLE_P(~A)))~%~A  ~
                      ~A = clm_add_gen_to_genbag(all_gens, ~%~A    ~
                         mus_make_file_to_frame_with_buffer_size((char *)(clm_int + CLM_ILOC(~A) + 2 + 2), clm_int[CLM_ILOC(~A) + 1]));~%"
	  indent iloc iloc indent
	  result indent
	  iloc iloc))
...
(defmacro <readin> (result rd)
  (when (> *safety* 0)
    (format *c-file* "  if (!(mus_readin_p(~A))) {mus_error(0, \"readin ~(~A~) is %s\", mus_describe(~A)); goto RUN_ALL_DONE;}~%"
	    (lc2 rd) (clean-arg rd) (lc2 rd)))
  (format *c-file* "  ~A = mus_readin(~A);~%" (lc2 result) (lc2 rd))
  nil)

(defmacro <readin?> (res arg)
  (format *c-file* "  ~A = mus_readin_p(~A);~%" (lc2 res) (lc2 arg))
  nil)

(defmethod gen-reflect ((gen readin) key val)
  (let ((name (lc key))
	(iadr (varinfo-iloc val)))
    (format *c-file* "  if (mus_readin_p(~A))~%    ~
                          {~%     ~
                           ~A = mus_increment(~A);~%     ~
                           ~A = mus_channel(~A);~%    ~
                          }~%"
  ...
  (format stream "~Aif (CLM_READIN_P(~A))~%~A  ~
                      ~A = clm_add_gen_to_genbag(all_gens, ~%~A    ~
                         mus_make_readin_with_buffer_size(~%~A      ~
                           ...
                           clm_to_off_t(clm_int, CLM_ILOC(~A) + 1), /* start */~%~A      ~
...
(defmacro <locsig-ref> (result loc chan)
  (when (> *safety* 0)
    (format *c-file* "  if (!(mus_locsig_p(~A))) {mus_error(0, \"locsig ~(~A~) is %s\", mus_describe(~A)); goto RUN_ALL_DONE;}~%"
...
(defmacro <locsig?> (res arg)
  (format *c-file* "  ~A = mus_locsig_p(~A);~%" (lc2 res) (lc2 arg))
  nil)
...
(defmacro <locsig> (result loc samp val)
  (when (> *safety* 0)
    (format *c-file* "  if (!(mus_locsig_p(~A))) {mus_error(0, \"locsig ~(~A~) is %s\", mus_describe(~A)); goto RUN_ALL_DONE;}~%"
	    (lc2 loc) (clean-arg loc) (lc2 loc)))
  (format *c-file* "  ~A = mus_locsig(~A, ~A, ~A);~%" (lc2 result) (lc2 loc) (lc-num-ref samp :integer) (lc-num-ref val))
  nil)
...
(defmacro <move-sound?> (res arg)
  (format *c-file* "  ~A = mus_move_sound_p(~A);~%" (lc2 res) (lc2 arg))
  nil)

(defmacro <move-sound> (result loc samp val)
  (when (> *safety* 0)
    (format *c-file* "  if (!(mus_move_sound_p(~A))) {mus_error(0, \"move-sound ~(~A~) is %s\", mus_describe(~A)); goto RUN_ALL_DONE;}~%"
	    (lc2 loc) (clean-arg loc) (lc2 loc)))
...
(defmacro <wave-train> (result s &optional fm)
  (when (> *safety* 0)
    (format *c-file* "  if (!(mus_wave_train_p(~A))) {mus_error(0, \"wave-train ~(~A~) is %s\", mus_describe(~A)); goto RUN_ALL_DONE;}~%"
	    (lc2 s) (clean-arg s) (lc2 s)))
  (if fm
      (format *c-file* "  ~A = mus_wave_train(~A, ~A);~%" (lc2 result) (lc2 s) (lc-num-ref fm))
    (format *c-file* "  ~A = mus_wave_train_1(~A);~%" (lc2 result) (lc2 s)))
  nil)

(defmacro <wave-train?> (res arg)
  (format *c-file* "  ~A = mus_wave_train_p(~A);~%" (lc2 res) (lc2 arg))
  nil)

(defmethod gen-reflect ((gen wave-train) key val)
  (let ((name (lc key))
	(iadr (varinfo-iloc val)))
    (format *c-file* "  if (mus_wave_train_p(~A))~%    ~
                          {~%     ~
                           ~A = mus_frequency(~A);~%    ~
                           ~A = mus_phase(~A);~%    ~
                          }~%"
...
(defmacro <filter?> (res arg)
  (format *c-file* "  ~A = mus_filter_p(~A);~%" (lc2 res) (lc2 arg))
  nil)

(defmacro <filter> (result s inval)
  (when (> *safety* 0)
    (format *c-file* "  if (!(mus_filter_p(~A))) {mus_error(0, \"filter ~(~A~) is %s\", mus_describe(~A)); goto RUN_ALL_DONE;}~%"
	    (lc2 s) (clean-arg s) (lc2 s)))
...
(defmacro <fir-filter> (result s inval)
  (when (> *safety* 0)
    (format *c-file* "  if (!(mus_fir_filter_p(~A))) {mus_error(0, \"fir-filter ~(~A~) is %s\", mus_describe(~A)); goto RUN_ALL_DONE;}~%"
	    (lc2 s) (clean-arg s) (lc2 s)))
  (format *c-file* "  ~A = mus_fir_filter(~A, ~A);~%" (lc2 result) (lc2 s) (lc-num-ref inval))
  nil)

(defmacro <fir-filter?> (res arg)
  (format *c-file* "  ~A = mus_fir_filter_p(~A);~%" (lc2 res) (lc2 arg))
  nil)
...
(defmacro <iir-filter> (result s inval)
  (when (> *safety* 0)
    (format *c-file* "  if (!(mus_iir_filter_p(~A))) {mus_error(0, \"iir-filter ~(~A~) is %s\", mus_describe(~A)); goto RUN_ALL_DONE;}~%"
	    (lc2 s) (clean-arg s) (lc2 s)))
  (format *c-file* "  ~A = mus_iir_filter(~A, ~A);~%" (lc2 result) (lc2 s) (lc-num-ref inval))
  nil)

(defmacro <iir-filter?> (res arg)
  (format *c-file* "  ~A = mus_iir_filter_p(~A);~%" (lc2 res) (lc2 arg))
  nil)
...
(defmacro <delay> (result s inval &optional pm)
  (when (> *safety* 0)
    (format *c-file* "  if (!(mus_delay_p(~A))) {mus_error(0, \"delay ~(~A~) is %s\", mus_describe(~A)); goto RUN_ALL_DONE;}~%"
	    (lc2 s) (clean-arg s) (lc2 s)))
  (if pm
      (format *c-file* "  ~A = mus_delay(~A, ~A, ~A);~%" (lc2 result) (lc2 s) (lc-num-ref inval) (lc-num-ref pm))
    (format *c-file* "  ~A = mus_delay_1(~A, ~A);~%" (lc2 result) (lc2 s) (lc-num-ref inval)))
  nil)
...
(defmacro <tap> (result s &optional pm) 
  ...
  (if pm
    ...
    (format *c-file* "  ~A = mus_tap_1(~A);~%" (lc2 result) (lc2 s)))
  nil)

(defmacro <delay?> (res arg)
  (format *c-file* "  ~A = mus_delay_p(~A);~%" (lc2 res) (lc2 arg))
  nil)
...
(defmacro <comb> (result s inval &optional pm)
  (when (> *safety* 0)
    (format *c-file* "  if (!(mus_comb_p(~A))) {mus_error(0, \"comb ~(~A~) is %s\", mus_describe(~A)); goto RUN_ALL_DONE;}~%"
	    (lc2 s) (clean-arg s) (lc2 s)))
  (if pm
      (format *c-file* "  ~A = mus_comb(~A, ~A, ~A);~%" (lc2 result) (lc2 s) (lc-num-ref inval) (lc-num-ref pm))
    (format *c-file* "  ~A = mus_comb_1(~A, ~A);~%" (lc2 result) (lc2 s) (lc-num-ref inval)))
  nil)

(defmacro <comb?> (res arg)
  (format *c-file* "  ~A = mus_comb_p(~A);~%" (lc2 res) (lc2 arg))
  nil)

(defmethod gen-reflect ((gen comb) key val)
  (let ((name (lc key))
	(iadr (varinfo-iloc val)))
    (format *c-file* "  if (mus_comb_p(~A))~%    ~
                          {~%     ~
                            ~A = mus_feedback(~A);~%    ~
                          }~%"
	    name
...
(defmacro <filtered-comb> (result s inval &optional pm)
  (when (> *safety* 0)
    (format *c-file* "  if (!(mus_filtered_comb_p(~A))) {mus_error(0, \"filtered-comb ~(~A~) is %s\", mus_describe(~A)); goto RUN_ALL_DONE;}~%"
	    (lc2 s) (clean-arg s) (lc2 s)))
  (if pm
      (format *c-file* "  ~A = mus_filtered_comb(~A, ~A, ~A);~%" (lc2 result) (lc2 s) (lc-num-ref inval) (lc-num-ref pm))
    (format *c-file* "  ~A = mus_filtered_comb_1(~A, ~A);~%" (lc2 result) (lc2 s) (lc-num-ref inval)))
  nil)

(defmacro <filtered-comb?> (res arg)
  (format *c-file* "  ~A = mus_filtered_comb_p(~A);~%" (lc2 res) (lc2 arg))
  nil)

(defmethod gen-reflect ((gen filtered-comb) key val)
  (let ((name (lc key))
	(iadr (varinfo-iloc val)))
    (format *c-file* "  if (mus_filtered_comb_p(~A))~%    ~
                          {~%     ~
                            ~A = mus_feedback(~A);~%    ~
                          }~%"
	    name
...
(defmacro <notch> (result s inval &optional pm)
  (when (> *safety* 0)
    (format *c-file* "  if (!(mus_notch_p(~A))) {mus_error(0, \"notch ~(~A~) is %s\", mus_describe(~A)); goto RUN_ALL_DONE;}~%"
	    (lc2 s) (clean-arg s) (lc2 s)))
  (if pm
      (format *c-file* "  ~A = mus_notch(~A, ~A, ~A);~%" (lc2 result) (lc2 s) (lc-num-ref inval) (lc-num-ref pm))
    (format *c-file* "  ~A = mus_notch_1(~A, ~A);~%" (lc2 result) (lc2 s) (lc-num-ref inval)))
  nil)

(defmacro <notch?> (res arg)
  (format *c-file* "  ~A = mus_notch_p(~A);~%" (lc2 res) (lc2 arg))
  nil)

(defmethod gen-reflect ((gen notch) key val)
  (let ((name (lc key))
	(iadr (varinfo-iloc val)))
    (format *c-file* "  if (mus_notch_p(~A))~%    ~
                          {~%     ~
                            ~A = mus_feedforward(~A);~%    ~
                          }~%"
	    name
...
(defmacro <all-pass> (result s inval  &optional pm)
  (when (> *safety* 0)
    (format *c-file* "  if (!(mus_all_pass_p(~A))) {mus_error(0, \"all-pass ~(~A~) is %s\", mus_describe(~A)); goto RUN_ALL_DONE;}~%"
	    (lc2 s) (clean-arg s) (lc2 s)))
  (if pm
      (format *c-file* "  ~A = mus_all_pass(~A, ~A, ~A);~%" (lc2 result) (lc2 s) (lc-num-ref inval) (lc-num-ref pm))
    (format *c-file* "  ~A = mus_all_pass_1(~A, ~A);~%" (lc2 result) (lc2 s) (lc-num-ref inval)))
  nil)

(defmacro <all-pass?> (res arg)
  (format *c-file* "  ~A = mus_all_pass_p(~A);~%" (lc2 res) (lc2 arg))
  nil)

(defmethod gen-reflect ((gen all-pass) key val)
  (let ((name (lc key))
	(iadr (varinfo-iloc val)))
    (format *c-file* "  if (mus_all_pass_p(~A))~%    ~
                          {~%     ~
                            ~A = mus_feedforward(~A);~%     ~
                            ~A = mus_feedback(~A);~%    ~
                          }~%"
...
(defmacro <moving-average> (result s inval)
  (when (> *safety* 0)
    (format *c-file* "  if (!(mus_moving_average_p(~A))) {mus_error(0, \"moving-average ~(~A~) is %s\", mus_describe(~A)); goto RUN_ALL_DONE;}~%"
	    (lc2 s) (clean-arg s) (lc2 s)))
  (format *c-file* "  ~A = mus_moving_average(~A, ~A);~%" (lc2 result) (lc2 s) (lc-num-ref inval))
  nil)

(defmacro <moving-average?> (res arg)
  (format *c-file* "  ~A = mus_moving_average_p(~A);~%" (lc2 res) (lc2 arg))
  nil)
...
(defmacro <env> (result e)
  (when (> *safety* 0)
    (format *c-file* "  if (!(mus_env_p(~A))) {mus_error(0, \"env ~(~A~) is %s\", mus_describe(~A)); goto RUN_ALL_DONE;}~%"
...
(defmacro <restart-env> (e)
  (when (> *safety* 0)
    (format *c-file* "  if (!(mus_env_p(~A))) {mus_error(0, \"(restart-)env ~(~A~) is %s\", mus_describe(~A)); goto RUN_ALL_DONE;}~%"
	    (lc2 e) (clean-arg e) (lc2 e)))
  (format *c-file* "  mus_reset(~A);~%" (lc2 e))
  nil)

(defmacro <env?> (res arg)
  (format *c-file* "  ~A = mus_env_p(~A);~%" (lc2 res) (lc2 arg))
  nil)

(defmacro <env-interp> (result x e &optional (base 1.0))
  (declare (ignore base))
  (when (> *safety* 0)
    (format *c-file* "  if (!(mus_env_p(~A))) {mus_error(0, \"env(-interp) ~(~A~) is %s\", mus_describe(~A)); goto RUN_ALL_DONE;}~%"
	    (lc2 e) (clean-arg e) (lc2 e)))
  ...
  (format stream "~Aif (CLM_ENV_P(~A))~%~A  ~
                      ~A = clm_add_gen_to_genbag(all_gens, ~%~A    ~
                         mus_make_env(~%~A      ~
                           ...
                           0.0, 0, /* dur, start both useless here */~%~A      ~
                           clm_int[CLM_ILOC(~A) + 3], /* end */~%~A      ~
...
(defmacro <start-function> (ctr type)
  (if (= type +as-needed-input+)
      (format *c-file* "static Float as_needed_~D(void *gen, int direction)~%" ctr)					       
    (if (= type +as-needed-edit+)
	(format *c-file* "static int as_needed_~D(void *closure)~%" ctr)				       
      (if (= type +as-needed-analyze+)
	  (format *c-file* "static bool as_needed_~D(void *closure, Float (*input)(void *arg1, int direction))~%" ctr)
	(format *c-file* "static Float as_needed_~D(void *closure)~%" ctr))))				       
  (format *c-file* "{~%")
  nil)
...
(defmacro <convolve?> (res arg)
  (format *c-file* "  ~A = mus_convolve_p(~A);~%" (lc2 res) (lc2 arg))
  nil)

(defmacro <convolve> (result s &optional func)
  (declare (ignore func))  
  (when (> *safety* 0)
    (format *c-file* "  if (!(mus_convolve_p(~A))) {mus_error(0, \"convolve ~(~A~) is %s\", mus_describe(~A)); goto RUN_ALL_DONE;}~%"
	    (lc2 s) (clean-arg s) (lc2 s)))
  ...
  (if (not ctr)
	...
	(format stream "        rd = clm_add_gen_to_genbag(all_gens, ~%~A          ~
                                  mus_make_readin(~%~A            ~
                                    ...
                                    clm_to_off_t(clm_int, CLM_ILOC(~A) + 6), /* start */~%~A              ~
                                    1)); /* direction */~%~A"
		indent
		indent
		iloc indent
...
(defmacro <src?> (res arg)
  (format *c-file* "  ~A = mus_src_p(~A);~%" (lc2 res) (lc2 arg))
  nil)

(defmacro <src> (result s &optional fm func)
  (when (> *safety* 0)
    (format *c-file* "  if (!(mus_src_p(~A))) {mus_error(0, \"src ~(~A~) is %s\", mus_describe(~A)); goto RUN_ALL_DONE;}~%"
	    (lc2 s) (clean-arg s) (lc2 s)))
  ...
  (if (not ctr)
	...
	(format stream "        rd = clm_add_gen_to_genbag(all_gens, ~%~A          ~
                                  mus_make_readin(~%~A            ~
                                    ...
                                    clm_int[CLM_ILOC(~A) + 4], /* chan */~%~A              ~
                                    clm_to_off_t(clm_int, CLM_ILOC(~A) + 5), /* start */~%~A              ~
                                    1)); /* direction */~%~A"
		indent
		indent
		iloc indent
...
(defmacro <granulate?> (res arg)
  (format *c-file* "  ~A = mus_granulate_p(~A);~%" (lc2 res) (lc2 arg))
  nil)

(defmacro <granulate> (result s &optional input-func edit-func)
  (declare (ignore input-func edit-func))    
  (when (> *safety* 0)
    (format *c-file* "  if (!(mus_granulate_p(~A))) {mus_error(0, \"granulate ~(~A~) is %s\", mus_describe(~A)); goto RUN_ALL_DONE;}~%"
	    (lc2 s) (clean-arg s) (lc2 s)))
...
(defmacro <phase-vocoder?> (res arg)
  (format *c-file* "  ~A = mus_phase_vocoder_p(~A);~%" (lc2 res) (lc2 arg))
  nil)

(defmacro <phase-vocoder> (result s &optional func1 func2 func3 func4)
  (declare (ignore func1 func2 func3 func4))
  (when (> *safety* 0)
    (format *c-file* "  if (!(mus_phase_vocoder_p(~A))) {mus_error(0, \"phase-vocoder ~(~A~) is %s\", mus_describe(~A)); goto RUN_ALL_DONE;}~%"
	    (lc2 s) (clean-arg s) (lc2 s)))
  ...
  (if (not input-ctr)
	...
	(format stream "        rd = clm_add_gen_to_genbag(all_gens, ~%~A          ~
                                  mus_make_readin(~%~A            ~
                                    ...
                                    clm_to_off_t(clm_int, CLM_ILOC(~A) + 7), /* start */~%~A              ~
                                    1)); /* direction */~%~A"
		indent
		indent
		iloc indent
...
(defmacro <phase-vocoder-phases> (res gen)
  (when (> *safety* 0)
    (format *c-file* "  if (!(mus_phase_vocoder_p(~A))) {mus_error(0, \"phase-vocoder ~(~A~) is %s\", mus_describe(~A)); goto RUN_ALL_DONE;}~%"
	    (lc2 gen) (clean-arg gen) (lc2 gen)))
  (format *c-file* "  ~A = mus_phase_vocoder_phases(~A);~%" (lc2 res) (lc2 gen))
  nil)
...
(defmacro <phase-vocoder-phase-increments> (res gen)
  (when (> *safety* 0)
    (format *c-file* "  if (!(mus_phase_vocoder_p(~A))) {mus_error(0, \"phase-vocoder ~(~A~) is %s\", mus_describe(~A)); goto RUN_ALL_DONE;}~%"
	    (lc2 gen) (clean-arg gen) (lc2 gen)))
  (format *c-file* "  ~A = mus_phase_vocoder_phase_increments(~A);~%" (lc2 res) (lc2 gen))
  nil)

(defmacro <phase-vocoder-outctr> (res gen)
  (when (> *safety* 0)
    (format *c-file* "  if (!(mus_phase_vocoder_p(~A))) {mus_error(0, \"phase-vocoder ~(~A~) is %s\", mus_describe(~A)); goto RUN_ALL_DONE;}~%"
	    (lc2 gen) (clean-arg gen) (lc2 gen)))
  (format *c-file* "  ~A = mus_phase_vocoder_outctr(~A);~%" (lc2 res) (lc2 gen))
  nil)

(defmacro <phase-vocoder-set-outctr> (res gen val)
  (when (> *safety* 0)
    (format *c-file* "  if (!(mus_phase_vocoder_p(~A))) {mus_error(0, \"phase-vocoder ~(~A~) is %s\", mus_describe(~A)); goto RUN_ALL_DONE;}~%"
	    (lc2 gen) (clean-arg gen) (lc2 gen)))
  (format *c-file* "  ~A = mus_phase_vocoder_set_outctr(~A, ~A);~%" (lc2 res) (lc2 gen) (lc-num-ref val :integer))
  nil)
...
(defmacro <mus-cosines> (res gen)
  (if (generator? gen)
      (format *c-file* "  ~A = mus_cosines(~A);~%" (lc2 res) (lc2 gen))
    (format *c-file* "  ~A = 0;~%" (lc2 res)))
  nil)

(defmacro <mus-set-cosines> (type gen val)
  (if (generator? gen)
      (if (= type +setf+)
	  (format *c-file* "  mus_set_cosines(~A, ~A);~%" (lc2 gen) (lc-num-ref val))
	(format *c-file* "  mus_set_cosines(~A, mus_cosines(~A) ~A ~A);~%"
		(lc2 gen) (lc2 gen) (if (= type +incf+) "+" "-") (lc-num-ref val))))
  nil)
...
(defmacro <mus-formant-radius> (result s)
  (if (generator? s)
      (format *c-file* "   ~A = mus_formant_radius(~A);~%" (lc2 result) (lc2 s))
    (format *c-file* "  ~A = 0.0;~%" (lc2 result)))
  nil)

(defmacro <mus-set-formant-radius> (type gen val)
  (if (generator? gen)
      (if (= type +setf+)
	  (format *c-file* "  mus_set_formant_radius(~A, ~A);~%" (lc2 gen) (lc-num-ref val))
	(format *c-file* "  mus_set_formant_radius(~A, mus_formant_radius(~A) ~A ~A);~%"
		(lc2 gen) (lc2 gen) (if (= type +incf+) "+" "-") (lc-num-ref val))))
      nil)
...
(defmacro <mus-name> (res gen)
  (format *c-file* "  ~A = mus_name(~A);~%" (lc2 res) (lc2 gen))
  nil)
...
(defmacro <mus-data-format-name> (res arg)
  (format *c-file* "  ~A = (char *)mus_data_format_name(~A);~%" (lc2 res) (lc2 arg))
  nil)
...
(defun method-type (method)
  (if (member method '(mus-length mus-channel mus-location mus-channels mus-order mus-cosines))
      'int
...
(defmacro <start> (end)
  ...
  (let* ((ints nil)
			   ...
			   (t
				...
				(if (eq (varinfo-gen-type lst) :mus-any-array)
				  ...
				  (progn
					...
					(if (or (eq key '*output*)
						(eq key '*reverb*))
					    (setf (varinfo-gen-type lst) 'frame->file)
					  (if (not (varinfo-temp lst))
    ...
    (if g-ints (format *c-file* "static off_t~{ ~A~^,~};~%" g-ints))
    ...
    (if ints (format *c-file* "  off_t~{ ~A~^,~};~%" ints))
    ...
    (format *c-file* "  off_t clm_beg, clm_end;~%")
    ...
    (format *c-file* "  clm_beg = clm_to_off_t(clm_int, ~D);~%" +clm-beg+)
    (format *c-file* "  clm_end = clm_to_off_t(clm_int, ~D);~%" +clm-end+)
		 ...
		 (let* ((c-name (lc key)))
		   ...
		   (cond ((eq (varinfo-type val) :integer)
			 ...
			 ((eq (varinfo-type val) :bignum)
			  (if (varinfo-iloc val)
			      (format *c-file* "  ~A = clm_to_off_t(clm_int, ~D);~%" c-name (varinfo-iloc val))))
...
(defmacro <end-1> ()
  ...
  (setf *safety* 0) ; don't try to goto RUN_ALL_DONE in the as-need-input functions
  nil)
