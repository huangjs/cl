...
(defun device-channels (device)
  (let* ((arr (make-array 1 :element-type 'short-float)))
    (clm-initialize-links)
    (read-audio-state device mus-audio-channel 0 arr)
    (floor (aref arr 0))))

(defun volume (&optional (device mus-audio-default) channel)
  (let* ((arr (make-array 1 :element-type 'short-float)))
    (clm-initialize-links)
    (if (not channel)
	(let ((chans (device-channels device)))
	  (loop for i from 0 below chans
	    do (read-audio-state device mus-audio-amp i arr)
	    collect (aref arr 0)))
      (progn
	(read-audio-state device mus-audio-amp channel arr)
	(aref arr 0)))))

(defsetf volume (&optional (device mus-audio-default) channel) (vals)
  `(let ((arr (make-array 1 :element-type 'short-float)))
     (clm-initialize-links)
     (if (not ,channel)
	 (let ((chans (device-channels ,device)))
	   (if (numberp ,vals)
	       (progn
		 (setf (aref arr 0) (coerce ,vals 'short-float))
		 (loop for i from 0 below chans do (write-audio-state ,device mus-audio-amp i arr)))
	     (loop for val in ,vals and i from 0 below chans do
	       (setf (aref arr 0) (coerce val 'short-float))
	       (write-audio-state ,device mus-audio-amp i arr))))
       (progn
	 (setf (aref arr 0) (coerce ,vals 'short-float))
	 (write-audio-state ,device mus-audio-amp ,channel arr)))
     ,vals))
...
(defun play (&optional name-1 &key start end (wait *clm-dac-wait-default*))
  ...
  (let ((filename (if name-1
	...
	(warn "no previous file to play")
      (if (functionp *clm-player*)                            ;for example (setf *clm-player* #'sl-dac)
...
(defun scale-to-file (out-file scaled-to &optional data-format stats header-type)
  (let* ((old-out-file out-file)
	 (ovals (or stats (float-sound-maxamp out-file))))
...
(defun begin-with-sound (srate channels scaled-to out-file revf statistics continue-old-file
  ...
  (let ((our-format (if scaled-to *clm-tempfile-data-format* data-format))
    ...
    (mus-set-clipping (if clipped 1 0))
    (mus-set-file-buffer-size *clm-file-buffer-size*)
    (if continue-old-file
	(progn
	  (clm-continue-output out-file)
	  (setf *output* (make-frame->file out-file channels our-format our-type)) ; for lisp-side methods, not for output
	  (when revf
	    (clm-continue-reverb revf)
	    (setf *reverb* (make-frame->file revf reverb-channels our-format our-type)))
	  out-file)
	...
	(let ((f (open out-file :direction :output :if-exists :supersede)))
	  (if f
	      (progn
		(close f)
		(clm-make-output out-file channels our-format our-type (or comment ""))  ; cmus.c make_frame_to_file
		(setf *output* (make-frame->file out-file channels our-format our-type)) ; for lisp-side methods, not for output
		(when revf
  ...
  `(let ((*verbose* ,verbose)
     ...
     (let* ((old-output *output*)
	   ...
	   (let* ((out-file (or *open-input-explicit-output*
		 ...
		 (tagbody
		  (restart-case
		   (catch :FINISH ,.body)	; mimic old style
		   (nil ()
...
(defun mix-wrapper (file output-sample input-file dur)
  (let ((frames (if dur (floor (* dur *srate*)) (sound-frames input-file))))
    (mix input-file :output (mus-file-name file) :output-frame output-sample :frames frames)
    (when *statistics*
	  ...
	  (if (or force-recomputation
		...
		(let* ((*open-input-explicit-output* (filename->string (make-typed-file-name fname sound-file-extension)))
		       ...
		       (*open-input-explicit-reverb* (filename->string 
						      (make-pathname
						       :defaults fname
						       :name (concatenate 'string (pathname-name fname) ".rev")
...
(def-optkey-fun mix (filename (input-frame 0) (output-frame 0) frames output)
  ...
  (let* ((outname (or output (mus-file-name *output*)))
    ...
    (clm-mix outname
	     filename
	     (round (+ output-frame *offset*))
	     (or frames (sound-frames filename))
	     input-frame)
    (setf *clm-ins* nil)
    (if old-output
	(progn
