...
(defun rmix (in-file &rest args &key (start-time 0.0) start reverb &allow-other-keys)
  (let* ((outname (mus-file-name *output*))
    ...
    (clm-mix outname in-file beg (sound-frames in-file) 0)
	...
	(if (not (= reverb 1.0))
	    (let ((tempname "mix-temp-reverb.snd"))
	      ...
	      (clm-mix revname tempname beg (sound-frames in-file) 0)
	      (delete-file tempname))
	  (clm-mix revname in-file beg (sound-frames in-file) 0)))
    (when old-output
...
;; the 96.3296 figure for max is from rmsp.ins
