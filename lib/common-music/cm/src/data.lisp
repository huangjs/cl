...
;;; generated by scheme->cltl from data.scm on 30-Jan-2007 07:38:50
...
(defparameter interval-names
  #((doubly-diminished ddim dd) (diminished dim d) (minor min mi m)
    (perfect perf p) (major maj ma mj) (augmented aug a)
...
(defun interval-quality-type (int)
  (if (%interval-encoded? int)
      (if (= (%interval-class int) interval-perfect-flag) 'perfect
          'imperfect)
      nil))

(defun interval-quality (int)
  (if (%interval-encoded? int)
      (car (elt interval-names (%interval-type int))) nil))
...
(defun interval-sign (int)
  (if (%interval-encoded? int) (%interval-sign int)
      (if (< int 0) -1 1)))
  ...
  (if (%interval-encoded? int)
      (let ((span (%interval-letters int))
        ...
        (dpb (+ 3 (* (- (%interval-type int) 3) -1))
             interval-type-byte
             (dpb (mod (- 7 span) 7) interval-letters-byte
                  (dpb (if (= span octs 0) 1 0) interval-octaves-byte
                       (dpb (mod (- 12 semi) 12)
                            interval-semitones-byte int)))))
      (mod (- 12 int) 12)))
...
(defun invert-interval (int)
  (if (%interval-encoded? int)
      (dpb
       (if (= (%interval-direction int) interval-down-flag)
           interval-up-flag interval-down-flag)
       interval-direction-byte int)
      (* int -1)))
...
(defun encode-interval (interval &optional letters)
  (let ((direction 1)
        ...
        (semitones nil)
        (octaves nil))
    (if letters (progn (setf quality interval))
        (progn
         (setf quality (car interval))
         (setf letters (cadr interval))))
    (unless (numberp letters)
      (error "~s is not a valid interval." interval))
    (if (= letters 0)
        (error
         "~s is not a one-based interval (1=Unison and 8=Octave)."
         interval)
        (if (< letters 0)
            (progn
             (setf direction -1)
    ...
    (let ((name (symbol-name quality)))
      (setf quality
              (do ((i 0 (+ i 1))
                   (e (length interval-names))
                   (f nil))
                  ((or f (= i e)) f)
                (if
                 (find-if
                  (lambda (y) (string-equal name (symbol-name y)))
                  (elt interval-names i))
                 (setf f i)))))
    (if (member letters '(0 3 4))
        (progn
         (setf class interval-perfect-flag)
         (case quality
           ...
           (t
            (error "~s invalid modifier for perfect interval ~s."
                   interval (+ 1 letters)))))
        (progn
         (setf class interval-majmin-flag)
         (case quality
...
(let ((entries
  ...
  (flet ((rsym (pre raw post)
           (intern
            (string-upcase
             (concatenate 'string (if (not pre) "" (symbol-name pre))
                          (if (stringp raw) raw (symbol-name raw))
                          (if (not post) ""
                              (if (stringp post) post
                                  (symbol-name post)))))
            :cm)))
    (dolist (e entries)
      (let ((rat (car e)))
...
(defmethod rhythm
           ((val number) &optional (tempo *tempo*) (beat *beat*))
           (unless (numberp beat)
             (setf beat (parse-rhythm-string (symbol-name beat))))
           (* (/ val beat) (/ 60 tempo)))

(defmethod rhythm
           ((val cons) &optional (tempo *tempo*) (beat *beat*))
               ...
               (let ((end (length str))
                 ...
                 (if
                  (and (> end 1)
                       (let ((c (elt str 1)))
                         (or (digit-char-p c)
                             (find-if (lambda (x) (char-equal c x))
                                      chars)))
                       (progn
                        (setf trp (char-equal (elt str 0) #\t))
                        (setf qup (char-equal (elt str 0) #\q))
                        (setf sep (char-equal (elt str 0) #\s))
                        (or trp qup sep)))
                  (setf beg 1) (setf beg 0))
                 (setf chr (elt str beg))
                 (setf pos
                         (position-if (lambda (x) (char-equal chr x))
                                      chars))
                 (cond
                  (pos (setf beg (+ beg 1))
                   (setf rhy (expt 2 (- pos 3))))
                  (t (setf pos beg)
                   (loop with flg = nil while (< beg end) do
                         (setf chr (elt str beg)) if
                         (or (digit-char-p chr)
                             (and (char= chr #\.) (not flg)
                                  (< beg (- end 1))
                                  (digit-char-p (elt str (+ beg 1)))
                                  (setf flg t)))
                         do (setf beg (+ beg 1)) else do (return))
                   (if (> beg pos)
                       (setf rhy
                               (read-from-string
                                (subseq str pos beg)))
                   ...
                   (when
                       (and (<= pos end)
                            (member (subseq str beg end)
                                    '("th" "TH" "nd" "ND" "st"
                                      "ST")))
                     (setf beg pos))))
                 (loop while
                       (and (< beg end) (char= (elt str beg) #\.))
                       count (progn (setf beg (+ beg 1))) into dot
                       finally
                       (unless (= dot 0)
                         (setf rhy (/ rhy (- 2 (expt 2 (- dot)))))))
                 (if trp (setf rhy (* rhy 3/2)))
                 (if qup (setf rhy (* rhy 5/4)))
                 (if sep (setf rhy (* rhy 7/4)))
                 (if (= beg end) rhy
                     (error "Can't parse ~A as a rhythm." str))))
             (next-token-position (str lb len)
               (loop with chr for i from lb below len do
                     (setf chr (elt str i)) until
                     (find-if (lambda (x) (char-equal chr x)) ops)
                     finally (return i))))
      (let* ((len (length rhythm))
             (lb 0)
             (ub (next-token-position rhythm lb len)))
        (unless (< lb ub) (error "Not a rhythm: ~A" rhythm))
        (loop with num =
              (/ 1 (parse-rhythm-token (subseq rhythm lb ub))) and
              val and op for i from 0 while (< ub len) do
              (setf op (elt rhythm ub)) (setf lb (+ ub 1))
              ...
              (cond
               ...
               ((char= op #\x)
                (setf val (read-from-string (subseq rhythm lb ub)))
                (setf num (loop repeat val collect num))))
              finally (return num))))))
...
(defparameter *logical-amplitudes*
  '(niente 0 pppp 1/10 ppp 1/5 pp 3/10 p 2/5 mp 1/2 mf 3/5 f 7/10 ff
    4/5 fff 9/10 ffff 1))

(defmethod amplitude
           ((amp symbol) &optional (softest *softest*)
...
(defmethod amplitude
           ((amp number) &optional (softest *softest*)
            (loudest *loudest*) (power *power*))
           (+ softest (* (- loudest softest) (expt amp power))))

(defmethod amplitude
           ((amp cons) &optional (softest *softest*)
            (loudest *loudest*) (power *power*))
           (mapcar (lambda (x) (amplitude x softest loudest power))
                   amp))
...
(defun interp1 (x coords base)
  (let ((head coords))
    (let* ((x1
            (if (null coords) (error "bad coordinate list: ~s" head)
                (pop coords)))
           (y1
            (if (null coords) (error "bad coordinate list: ~s" head)
                (pop coords)))
...
(defun interpl (x coords &key min max (offset min) scale (base 1))
  ...
  (let ((y (interp1 x coords base)))
    (if offset (+ (if scale (* y scale) y) offset)
        (if scale (* y scale) y))))
...
(defun tendency
       (x low high
        &key min max (offset min) scale (ranfn #'random)
        (state *random-state*)
        &aux value)
  (if (and offset max) (setf scale (- max offset)))
    ...
    (if (and (or x-min x-max) (or y-min y-max))
        ...
        (if (or x-min x-max)
            (progn
             ...
             (loop for tail on env by #'cddr for x = (car tail) for y
                   = (cadr tail) collect
                   (rescale x minx maxx x-min x-max) collect y))
            (progn
             (or y-min (setf y-min miny))
             (or y-max (setf y-max maxy))
             (loop for tail on env by #'cddr for x = (car tail) for y
                   = (cadr tail) collect x collect
                   (rescale y miny maxy y-min y-max)))))))
...
(defun mymod (num div)
  (if (and (integerp num) (integerp div)) (mod num div)
...
(defun between (lb ub &optional exception (state *random-state*))
  (let ((range (- ub lb)))
    (if (not (> range 0)) lb
        (if exception
            (loop for num = (+ lb (random range state)) when
                  (not (= exception num)) return num)
...
(defun pickl
       (seq
        &key (end (length seq)) (start 0) avoid
        (state *random-state*))
  (let ((range (- end start)))
    (if avoid
        (loop with x = avoid while (eq x avoid) do
              (setf x (elt seq (+ start (random range state))))
              finally (return x))
        (elt seq (+ start (random range state))))))

(defun vary
       (value variance
        &optional (where :around) (state *random-state*))
  (if (or (<= variance 0) (= value 0)) value
...
(defun drunk
       (n width
        &key (low most-negative-fixnum) (high most-positive-fixnum)
        (mode :reflect) (avoid nil) (state *random-state*))
  (let ((amt (between (- width) (+ width 1) avoid state)))
    ...
    (unless (<= low n high)
      (cond ((eq mode ':reflect) (setf n (fit n low high)))
            ...
            (t
             (error
              "~s is not :refect :reset :jump  :limit or :stop."
              mode))))
    n))

(defun odds
       (n
        &optional (if-true-val t) (if-false-val nil)
        (state *random-state*))
  (if (< (random 1.0 state) n) if-true-val if-false-val))

(defun shuffle
       (seq
        &key (start 0) (end (length seq)) (state *random-state*)
        (copy t)
        &aux (width (- end start)))
  (if (< width 2) seq
      (progn
       (when copy (setf seq (copy-list seq)))
       (loop for i from start to (- end 1) for j =
             (+ start (random width state)) for v = (elt seq i) do
             (setf (elt seq i) (elt seq j)) (setf (elt seq j) v))
       seq)))

(defun ran
       (&key (type (quote :uniform)) from (below 1.0)
        (state *random-state*) a b distribution)
  (let ((num nil) (res nil))
    (if (and from below)
        (progn
         (when (< below from) (rotatef below from))
         (setf num (- below from)))
        (setf num
                (or below
                    (error
                     "Missing ':below' (upper bound value)."))))
    (flet ((betad (a b s)
             (loop with |1/A| = (/ 1 a) and |1/B| = (/ 1 b) for r1 =
                   (random 1.0 s) for r2 = (random 1.0 s) for y1 =
                   (expt r1 |1/A|) for y2 = (expt r2 |1/B|) for sum =
                   (+ y1 y2) while (> sum 1.0) finally
                   (return (/ y1 sum))))
           (gaussian (s)
           ...
           (exp2 (l s)
             (let ((u nil) (v nil) (2*e^-1 (* 2 (exp -1.0))))
               (loop do (setf u (- 1.0 (random 1.0 s)))
                     (setf v (* 2*e^-1 (random 1.0 s))) until
                     (<= v (* -2.0 u (log u))) finally
                     (return (/ v u l)))))
           (cauchy (pos? s)
           ...
           (cheezy-gamma (nu s)
             (let ((r 1))
               (loop repeat (round nu) do
                     (setf r (* r (- 1 (random 1.0 s)))))
               (- (log r)))))
      (case (or distribution type)
        ((:uniform :u uniform u) (setf res (random num state)))
        ((:low :lp :low-pass lp low-pass)
         (setf res (min (random num state) (random num state))))
        ((:high :hp :high-pass hp high-pass)
         (setf res (max (random num state) (random num state))))
        ((:mean :triangular :bp :band-pass bp band-pass)
         (setf res (/ (+ (random num state) (random num state)) 2)))
        ((:beta beta)
         (if a (if b nil (setf b a))
             (if b (setf a b) (progn (setf a 0.5) (setf b 0.5))))
         (setf res (* num (betad a b state))))
        ((:slow-exp slow-exp) (setf res (* num (exp2 a state))))
        ((:exponential exponential :exp exp)
         (setf res (* num (/ (- (log (- 1 (random 1.0 state)))) a))))
        ((:cauchy cauchy) (setf res (* num (cauchy a state))))
        ((:poisson poisson) (setf res (* num (poisson a state))))
        ((:gamma gamma) (setf res (* num (cheezy-gamma a state))))
...
(defun ransegs
       (num
        &key (type ':uniform) (min 0.0 mnp) (max 1.0 mxp)
        (sum 1.0 smp) a b)
  (if smp (setf max sum))
  (if (not (< min max))
...
(defun expl (power &key (y0 0.0) (y1 1.0) (base 2))
  ...
  (if (= base 1) (+ y0 (* (- y1 y0) power))
      (+ y0
         (* (/ (- y1 y0) (- base 1.0)) (- (expt base power) 1.0)))))

(defun explsegs (len sum power)
  (if (= len 0) '()
      (loop for i from 1 to len for l = 0 then v for v =
            (expl (/ i len) :base power) collect (* sum (- v l)))))
...
(defun geosegs (len sum base)
  (if (= len 0) nil
      (let ((a (* sum (/ (- 1 base) (- 1 (expt base len))))))
        (loop for n from 0 below len collect a do
              (setf a (* a base))))))
...
(defun markov-analyze
       (seq
        &key (order 1) (print? t) (pattern? t) sort?
        (print-decimals 3) key)
  (let ((len (length seq))
        (labels '())
        (table '())
        (row-label-width 8)
    ...
    (labels ((add-outcome (prev next)
               (let ((entry
                 ...
                 (if (not entry)
                     (push
                      (list prev (format nil "~s" prev)
                            (list next 1))
                      table)
                     (let ((e (assoc next (cddr entry))))
                       (if e (rplaca (cdr e) (+ 1 (cadr e)))
                       ...
                       (setf a
                               (concatenate 'string a
                                            (format nil " ~s"
                                                    (car x)))))))))
      (dotimes (i len)
        (loop with prev = (list) for j to order for x =
              (let ((raw (elt seq (mod (+ i j) len))))
                (if key (funcall key raw) raw))
              when (< j order) do (push x prev) finally
              (progn
               (add-outcome (reverse prev) x)
               (if (not (member x labels)) (push x labels)))))
      ...
      (loop for row in table for lab = (cadr row) for val =
            (cddr row) maximize (length lab) into len do
            (let ((total (loop for e in val sum (cadr e))))
              (setf total (* total 1.0))
              (loop for e in val do
                    (rplaca (cdr e)
      ...
      (when (member print? '(t table :table))
        (let* ((sp " ") (ln (make-string field :initial-element #\-)))
          (progn
           (terpri)
           ...
           (dolist (l labels)
             ...
             (let* ((s (format nil "~a" l)) (n (length s)))
               (dotimes (i (max (- field n) 0)) (write-char #\ ))
               (princ s))))
          (dolist (row table)
            (terpri)
            ...
            (dolist (l labels)
              (let ((v (assoc l (cddr row))))
                (if (not v) (progn (princ sp) (princ ln))
      ...
      (if pattern? (make-instance (find-class 'markov) :of pat)
          (values)))))

(defun histogram (numbers lo hi slots)
  (let ((hist (make-list slots :initial-element 0)) (rang (- hi lo)))
    (loop with i for x in numbers if (and (>= x lo) (< x hi)) do
          (setf i (floor (* slots (/ (- x lo) rang))))
