...
;;; generated by scheme->cltl from loop.scm on 30-Jan-2007 07:38:47
...
(defun parse-numerical-for (forms clauses ops)
  ...
  (let ((var (cadr forms))
    ...
    (if (or (not from) (numberp from))
        (push (make-binding var (or from 0)) bind)
        (progn
         (push (make-binding var nil) bind)
         (push `(setf ,var ,from) init)))
    (if (not step) (setf step 1)
        (if (not (numberp step))
            (let ((var (gensym "v")))
              (push (make-binding var nil) bind)
              (push `(setf ,var ,step) init)
              (setf step var))))
    (setf step `(setf ,var (,incr ,var ,step)))
    (if stop
        (let ((end (gensym "v")))
          (if (numberp stop) (push (make-binding end stop) bind)
              (progn
               (push (make-binding end nil) bind)
               (push `(setf ,end ,stop) init)))
          (setf stop (list test var end))))
    (values
     (make-loop-clause 'operator 'for 'bindings (reverse bind)
      'initially (reverse init) 'stepping (list step) 'end-tests
...
(defun parse-sequence-iteration (forms clauses ops)
  ...
  (let ((head forms)
    ...
    (do ((next nil))
      ...
      (case next
        ((in on across over)
         ...
         (setf data (pop tail)))
        ((by)
         (if incr (loop-error ops head "Duplicate 'by'.")
             (if (eq type 'across)
                 (loop-error ops head
    ...
    (cond
     ...
     ((eq type 'over)
      (let ((getit
             (if incr `(setf ,var (next ,seq ,incr))
                 `(setf ,var (next ,seq)))))
     ...
     (t
      ...
      (push
       (if (eq type 'in) `(setf ,var (car ,seq)) `(setf ,var ,seq))
       loop)
      (push `(null ,seq) stop)))
    (values
     (make-loop-clause 'operator 'for 'bindings (reverse bind)
  ...
  (let ((head forms)
    ...
    (values
     (make-loop-clause 'operator 'for 'bindings
      ...
      (if init (list init) '()) 'looping (if loop (list loop) '())
...
(defun parse-with (forms clauses ops)
  ...
  (let ((head forms)
      ...
      (cond
       ((and (loop-variable? next) need)
        ...
        (if expr
            (loop-error ops head "Found '" next
             "' where 'and' expected."))
        (setf var next) (setf expr nil) (setf and? nil)
        (setf need nil))
       ((eq next 'and)
        (if and? (loop-error ops head "Duplicate 'and'.")
            (if var
                (if expr
                    (progn
                     (push (make-binding var nil) bind)
       ...
       ((eq next '=)
        (if expr
            (loop-error ops head "Found '=' where 'and' expected.")
            (setf expr (pop tail))))
       (t
        (if need
            (loop-error ops head "Found '" next
    ...
    (if and? (loop-error ops head "Extraneous 'and'.")
        (if var
            (if expr
                (progn
                 (push (make-binding var nil) bind)
...
(defun compatible-accumulation? (typ1 typ2)
  (let ((l1 '(collect append nconc))
    ...
    (or (eq typ1 typ2) (and (member typ1 l1) (member typ2 l1))
...
(defun parse-accumulation (forms clauses ops)
  (flet ((make-collector (var type acc head)
    ...
    (let ((save forms)
      ...
      (if (not coll) (setf new? t)
          (if
           (not
            (compatible-accumulation? oper (collector-type coll)))
      ...
      (case oper
        ((sum count)
         (let ((acc nil))
           ...
           (if (eq oper 'sum) (push `(setf ,acc (+ ,acc ,expr)) loop)
        ...
        ((minimize maximize)
         (let ((var (gensym "v"))
           ...
           (push
            `(progn
              (setf ,var ,expr)
              (if (or (not ,acc) (,opr ,var ,acc)) (setf ,acc ,var)))
            loop)))
        ((append collect nconc)
         (let ((head nil) (tail nil))
           (if (not new?)
               (progn
                (setf tail (collector-acc coll))
               ...
               (progn
                ...
                (setf coll (make-collector into oper tail head))
                (if (not into) (push `(cdr ,head) return))))
           (if (eq oper 'append)
               (progn
                (push `(rplacd ,tail (append ,expr (list))) loop)
                (push `(setf ,tail (last ,tail)) loop))
               (if (eq oper 'collect)
                   (progn
                    (push `(rplacd ,tail (list ,expr)) loop)
                    (push `(setf ,tail (cdr ,tail)) loop))
                   (progn
                    (push `(rplacd ,tail ,expr) loop)
                    (push `(setf ,tail (last ,tail)) loop))))
           (if into (push `(setf ,into (cdr ,head)) loop)))))
      (values
       (make-loop-clause 'operator oper 'bindings (reverse bind)
        'initially (reverse init) 'looping (reverse loop) 'returning
        (reverse return) 'collectors (if new? (list coll) '())
...
(defun parse-while-until (forms clauses ops)
  ...
  (let ((head forms) (oper (pop forms)) (test nil) (stop '(go t)))
    ...
    (values
     (make-loop-clause 'operator oper 'looping
      (list `(if ,test ,stop)))
     forms)))
...
(defun parse-thereis (forms clauses ops)
  ...
  (let ((oper (car forms)) (expr nil) (bool nil) (func nil))
    ...
    (setf forms (cddr forms))
    (values
     (make-loop-clause 'operator 'thereis 'looping
...
(defun parse-return (forms clauses ops)
  ...
  (let ((oper (car forms)) (expr nil) (func nil))
    ...
    (setf func (cadddr (loop-op? oper ops)))
    (values
     (make-loop-clause 'operator 'return 'looping
      `(,(funcall func expr)))
     forms)))
...
(defun legal-in-conditional? (x ops)
  (let ((op (loop-op? x ops)))
    (if
     (and op (not (null (cddr op))) (eq (caddr op) 'task)
              ...
              (progn
               ...
               (if (null forms)
                   (loop-error ops previous
                    "Missing 'and' clause.")))
              (if (eq (car forms) 'else) (setf stop? t)
...
(defun parse-conditional (forms clauses ops)
  (let ((ops (cons '(else) ops))
    ...
    (if (eq oper 'unless) (setf expr (list 'not expr)))
    (multiple-value-bind
        (a b)
    ...
    (setf loop
            (list 'if expr
                  (append `(progn ,@(loop-looping then)) (list))
                  nil))
    (if (and (not (null forms)) (eq (car forms) 'else))
        (progn
         (setf forms (cdr forms))
         (when (null forms)
     ...
     (let ((previous forms) (clauses '()))
       (do ((op nil)
         ...
         (multiple-value-bind
           ...
           (setf clause a)
           (setf remains b))
         (if (op-type? op 'task) (setf body op)
             (if (op-type? op 'iter)
                 (if (not (null body))
...
(defparameter *loop-operators*
  (list (list 'with #'parse-with nil)
        (list 'initially #'parse-initially nil)
        (list 'repeat #'parse-repeat 'iter)
        (list 'for #'parse-for 'iter
              ...
              (list 'on #'parse-sequence-iteration)
              (list 'across #'parse-sequence-iteration)
              (list '= #'parse-general-iteration)
              (list 'over #'parse-sequence-iteration))
        (list 'as #'parse-for 'iter) (list 'do #'parse-do 'task)
        (list 'collect #'parse-accumulation 'task)
        (list 'append #'parse-accumulation 'task)
        (list 'nconc #'parse-accumulation 'task)
        (list 'sum #'parse-accumulation 'task)
        (list 'count #'parse-accumulation 'task)
        (list 'minimize #'parse-accumulation 'task)
        (list 'maximize #'parse-accumulation 'task)
        (list 'thereis #'parse-thereis 'task #'loop-return)
        (list 'always #'parse-thereis 'task #'loop-return)
        (list 'never #'parse-thereis 'task #'loop-return)
        (list 'return #'parse-return 'task #'loop-return)
        (list 'while #'parse-while-until nil)
        (list 'until #'parse-while-until nil)
        (list 'when #'parse-conditional 'task)
        (list 'unless #'parse-conditional 'task)
      ...
      (block nil
        (tagbody
         ...
         ,@(let ((tests (loop-end-tests iter)))
             (if tests
                 (list
                  `(if ,(if (cdr tests) (cons 'or tests) (car tests))
                       (go t)))
                 (list)))
         ,@(loop-looping iter)
         ,@(loop-stepping iter)
