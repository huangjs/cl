...
;;; generated by scheme->cltl from midi2.scm on 30-Jan-2007 07:39:03
...
(progn
 (defclass midi-stream-mixin nil
 ...
 (defparameter <midi-stream-mixin> (find-class 'midi-stream-mixin))
...
(progn
 (defclass midi-file (event-file midi-stream-mixin)
           ((elt-type :initform :byte :initarg :elt-type :accessor
            ...
            (timesig :initform nil :initarg :timesig :accessor
             midi-file-timesig)
            (tempo :initform *midi-file-default-tempo* :initarg
             :tempo :accessor midi-file-tempo)
            ...
            (format :initform 0 :initarg :format :accessor
                    midi-file-format)
...
(defparameter *midi-player*
  (let ((os (os-name)))
    (cond ((member os '(unix linux cygwin)) "timidity -quiet=2")
          ((member os '(darwin osx macosx))
           (cond
            ((probe-file "/usr/local/bin/timidity")
...
(defun midi-file-write-header
       (mf fmat tracks division &rest resolution)
  (let ((fp (io-open mf)))
    (write-bytes fp +mthd+ 4)
    (write-bytes fp 6 4)
    (write-bytes fp fmat 2)
    (write-bytes fp tracks 2)
    (if (null resolution) (write-bytes fp division 2)
        (progn
         (write-bytes fp (+ (- division) 256) 1)
    ...
    (cond
     ((< byte 240)
      (if (logtest byte 128)
          ...
          (progn
           (setf raw (logior (ash (midi-file-status mf) 8) byte))
     ...
     ((= byte +ml-file-meta-marker+) (setf byte (read-byte fp))
      (cond
       ((= byte +ml-file-eot-opcode+) (read-byte fp)
        (multiple-value-bind
            (m d)
            (make-eot)
       ...
       ((= byte +ml-file-tempo-change-opcode+)
        ...
        (let ((usecs
               (logior (ash (read-byte fp) 16) (ash (read-byte fp) 8)
                       (read-byte fp))))
          (multiple-value-bind
              (m d)
       ...
       ((= byte +ml-file-time-signature-opcode+)
        (let ((len (read-variable-quantity fp)))
          (unless (= len 4)
            (error "unexpected time signature length: ~s" len))
          (multiple-value-bind
              (m d)
            ...
            (setf (midi-file-data mf) d)
            (values m))))
       (t (setf size (read-variable-quantity fp))
        (multiple-value-bind
            (m d)
...
(defmethod midi-write-message ((msg number) (mf midi-file) time data)
           (let ((fp (io-open mf))
             ...
             (cond
              ((< 0 type 15)
               (write-byte
                (logior (ldb +enc-lower-status-byte+ msg)
                        (ash (ldb +enc-upper-status-byte+ msg) 4))
                fp)
               (when (> size 1)
                 (write-byte (ldb +enc-data-1-byte+ msg) fp))
               (when (> size 2)
                 (write-byte (ldb +enc-data-2-byte+ msg) fp)))
              ((= type +ml-meta-type+) (write-byte 255 fp)
               (write-byte (ldb +enc-data-1-byte+ msg) fp)
               (loop for i below (length data) do
                     (write-byte (elt data i) fp)))
              ((= type 15)
               (let ((byt
                      (logior (ash type 4)
...
(defun midi-file-set-track (mf track)
  (let ((fil (io-open mf)))
    ...
    (loop for p = +midi-file-header-length+ then
          (+ p 8 (midi-file-read-track-header mf)) for c from 0 do
          (set-file-position fil p t) while (< c track))
    (setf (midi-file-tracklen mf) (midi-file-read-track-header mf))
    track))

(defun channel-tuning-init (io)
  (let ((tuning (midi-stream-channel-tuning io)))
    (if (not tuning) (progn (setf (midi-stream-tunedata io) '()))
        (let ((tune nil) (num1 nil) (num2 nil) (data nil) (type nil))
          (if (consp tuning) (setf tune (pop tuning))
              (progn (setf tune tuning) (setf tuning nil)))
          (setf type
                  (find-if (lambda (x) (member tune x))
                           %midituningtypes))
          (cond
           ((eq type (car %midituningtypes))
            (if (consp tuning)
                (progn
                 (setf num1 (pop tuning))
            ...
            (unless (<= 0 num1 num2 15)
              (error "tuning range ~s-~s not in channel range 0-15."
                     num1 num2))
            (setf data
                    (list t (- num2 num1) (- num2 num1) num1
...
(defun microtune-channels
       (io divisions
        &optional (width *midi-pitch-bend-width*) (channel-offset 0))
  (if (= divisions 1)
      (loop for c below 16 for m =
...
(defmethod open-io :after ((mf midi-file) dir &rest args) args
           (if (eq dir ':output)
           ...
           (when (eq (io-direction mf) ':output)
             (let ((msg nil) (data nil))
               ...
               (when (midi-file-timesig mf)
                 (multiple-value-setq (msg data)
                   (apply #'make-time-signature
                          (midi-file-timesig mf)))
                 (midi-write-message msg mf 0 data))
               (when (midi-file-keysig mf)
                 (multiple-value-setq (msg data)
                   (apply #'make-key-signature
                          (midi-file-keysig mf)))
                 (midi-write-message msg mf 0 data))
...
(defmethod close-io :before ((mf midi-file) &rest mode) mode
           (when (eq (io-direction mf) ':output)
             (multiple-value-bind
                 (m d)
                 (make-eot)
        ...
        (progn
         (with-open-io (io file :input)
          (format stream
                  ...
                  file (midi-file-format io) (midi-file-tracks io)
                  (midi-file-divisions io))
          (midi-file-set-track io track)
          (format stream "~%Track ~s, length ~s~%"
                  (midi-file-track io) (midi-file-tracklen io))
          (midi-file-map-track
           (lambda (mf)
