...
;;; generated by scheme->cltl from objects.scm on 30-Jan-2007 07:38:49
...
(defun save-object (obj file)
  (let ((fp nil))
    (unwind-protect
        (progn
         (setf fp (open-file file :output))
         (if (consp obj)
             (dolist (o obj) (write (make-load-form o) :stream fp))
...
(progn
 (defclass container nil
           ((name :initform nil :accessor object-name :initarg
             :name)))
...
(defmethod initialize-instance :after ((obj container) &rest args)
           ...
           (let ((name (object-name obj)))
             (when name
               ...
               (setf (gethash (string-downcase name) *dictionary*)
                       obj))
             (values)))
             ...
             (if old
                 (if (eq obj old) old
                     (if err?
                         ...
                         nil))
                 (progn
                  (remhash (object-name obj) *dictionary*)
...
(defun list-named-objects (&optional type)
  (hash-fold
   (if type (lambda (k v p) k (if (typep v type) (cons v p) p))
               ...
               (let* ((key (object-name val))
                 ...
                 (if typ
                     (let ((nam (filename-name key))
                       ...
                       (if
                        (and (string= type typ) (string= name nam)
                             (or (not path) (string= path dir)))
                        (if find
                            (error "More than one file named ~S."
                                   string)
                            (setf find val)))))))
             nil)
           nil *dictionary*)))
...
(defmethod subcontainers ((obj seq))
           (loop for o in (container-subobjects obj) when
                 (typep o <container>) collect o))
...
(defun map-objects
  ...
  (when (or slot slot!)
    (if key (error ":slot[!] and :key are exclusive keywords.")
        (setf key
                (if slot! (lambda (x) (slot-value x slot!))
  ...
  (do ((tail (nthcdr start objs) (nthcdr step tail))
    ...
    (cond
     ((> width 1)
      (setf this
              (loop for i below width for x = (nthcdr i tail) until
                    (null x) collect (car x)))
      (when
          (or (not class)
              (loop for x in this always (typep x class)))
        (if key (setf data (mapcar key this)) (setf data this))
        (if
         (or (not test) (loop for x in data always (funcall test x)))
         (if slot!
             (loop for x in (funcall func data) for y in this do
                   (setf (slot-value y slot!) x))
             (funcall func data)))))
     (t (setf this (car tail))
      (when (or (not class) (typep this class))
        ...
        (if (or (not test) (funcall test data))
            (if slot!
                ...
                (funcall func data))))))
    (when doat
      (if (null at) (progn (setf done t) (setf step 0))
          (progn (setf step (- (pop at) indx)))))
    (setf indx (+ indx step))))

(defun fold-objects (fn objects acc &rest args)
  (apply #'map-objects (lambda (x) (setf acc (funcall fn x acc)))
         objects args)
  acc)

(defun subobjects (object &rest args)
  (if (null args) (container-subobjects object)
...
(defun list-objects (object &rest args)
  (apply #'map-objects (lambda (x i) (format t "~d. ~s~%" i x))
...
(progn
 (defclass event nil ((time :accessor object-time :initarg :time)))
               ...
               (let ((class (class-of obj)) (*print-case* ':downcase))
                 ...
                 (do ((slots (class-slots class) (cdr slots))
                   ...
                   (if (slot-boundp obj s)
                       (progn
                        (setf v (slot-value obj s))
                        (setf k (slot-definition-initargs d))
                        (unless (null k)
                          (unless
                              (and (eq *print-event* ':terse)
                                   (eq v
                                       (slot-definition-initform d)))
                            (format port " ~a ~s" s v))))))
                 (format port ")")
                 obj)
               (call-next-method)))
...
(defun insure-parameters (pars decl supers)
  (flet ((getslotd (slot sups)
           (do ((tail sups (cdr tail))
             ...
             (setf isit
                     (find-if
                      (lambda (x) (eq slot (slot-definition-name x)))
  ...
  (flet ((gettimepar (slot sups)
           (do ((tail sups (cdr tail))
             ...
             (if pars
                 (let ((test
                        (find-if
                         (lambda (x) (eq slot (parameter-slot x)))
                         pars)))
...
(defmacro defobject (name supers slots &body options)
  (let ((sups
         (mapcar
          (lambda (x)
            (or (find-class x) (error "No class named ~s." x)))
          supers))
...
(defun parse-each (forms clauses ops)
  ...
  (let ((save forms)
    ...
    (setf loop
            (list
             `(,'do (,@(loop-bindings each))
               (,(if (null (cdr ends)) (car ends) `(or ,@ends)) nil)
...
(defparameter *process-operators*
  (append
   (mapcar (lambda (op) (assoc op *loop-operators*))
           '(with initially repeat for as do finally when unless if))
   (list (list 'set #'parse-set-clause 'task)
...
(defun bang! (box &rest args)
  (let ((pmode ':bang!))
    (cond ((null args) nil)
          ...
          ((eq (car args) ':send!) (setf pmode ':send!)
           (setf args (cdr args)))
          ((eq (car args) ':stop!) (setf pmode ':stop!)) (t nil))
    (if (eq pmode ':stop!) (values)
        (progn
         (if (not (null args))
             (if (eq (car args) ':argn)
                 (if (null (cdr args)) (setf (elt box 1) (list))
