...
;;; generated by scheme->cltl from patterns.scm on 30-Jan-2007 07:38:55
...
(defparameter +nad+ ':not-a-datum)

(defparameter +eop+ ':end-of-period)

(defparameter +eod+ ':end-of-data)
...
(progn
 (defclass pattern (container)
           ((flags :initarg :flags :initform 0 :accessor
            ...
            (parser :initarg :parser :initarg :tempo :initarg
             :through :initarg :from :initarg :in :initform nil
...
(defmethod canonicalize-pattern-data
           ((obj pattern) data parser inits) obj inits
           (if parser
               (let ((coll (list nil)))
                 (do ((tail data (cdr tail))
                      ...
                      (num 0)
                      (lst coll))
                     ((null tail) (values (cdr coll) len (= num 0)))
                   (if (pattern? (car tail)) (setf num (+ num 1)))
                   (rplacd lst (list (maybeparse parser (car tail))))
                   (setf lst (cdr lst))))
               (values data (length data)
                       (not (some #'pattern? data)))))
...
(defmethod pattern-external-inits ((obj pattern))
           (let ((inits
                  (slot-init-forms obj :eval t :ignore-defaults t
                   :only
                   '(repeat parser returning counting traversing)
...
(defun expand-pattern-value (val)
  (cond ((pattern? val) (make-load-form val))
        ((consp val)
         (if (eq (first val) 'quote) val
             (cons 'list
                   (loop for x in val collect
                         (expand-pattern-value x)))))
        ((and val (symbolp val)) `',val) (t val)))

(defun pattern-value-function (etyp opt val parse?)
  (case etyp
    ((:notes notes)
     (if parse? (if opt #'keynum #'note)
         (case opt
           ((:in in) (lambda (x) (note x :in val)))
           ((:through through) (lambda (x) (note x :through val)))
           (t nil))))
    ((:keynums keynums)
     (if parse? #'keynum
         (case opt
           ((:from from) (lambda (x) (keynum x :from val)))
           ((:through through) (lambda (x) (keynum x :through val)))
           ((:to to) (lambda (x) (keynum x :to val)))
           (t nil))))
    ((:rhythms rhythms)
     (if parse?
         (if (not opt) #'rhythm
             (if (numberp val) (lambda (x) (rhythm x val)) nil))
         (if (or (not opt) (numberp val)) nil
             (lambda (x) (rhythm x (next-1 val))))))
    ((:hertz hertz)
     (if parse? (if opt #'keynum #'hertz)
         (case opt
           ((:in in) (lambda (x) (hertz x :in val)))
           ((:through through)
            (lambda (x) (hertz x :through val))))))
    ((:amplitudes amplitudes) (if parse? #'amplitude nil))
    (t nil)))
...
(defmethod initialize-instance :after ((obj pattern) &rest args) args
           (let ((flags (pattern-flags obj))
             ...
             (unless (pattern-returning obj)
               (let ((ret
                      (pattern-value-function type optn valu nil)))
             ...
             (let* ((default (default-period-length obj))
               ...
               (setf (pattern-period obj)
                       (if (or (numberp period) (eq period t))
                           (make-period :length period :default
                            default :hook hook)
                           (make-period :stream period :default
                            default :hook hook))))
...
(defmethod initialize-instance :after ((obj cycle) &rest args) args
           (let ((cyc (make-cycl)))
            ...
            (list ':of
                  (cons 'list
                        (loop for x in (car (pattern-data obj))
                              collect (expand-pattern-value x))))
            (call-next-method)
            (if (not (palindrome-elide obj)) (list)
                (list ':elide
                      (expand-pattern-value
           ...
           (let ((cycl (pattern-data obj)))
             (when (null (cycl-tail cycl))
               (let ((mode (next-1 (palindrome-elide obj)))
                 ...
                 (cond
                  ((member mode '(t 2) :test #'eq)
                   (setf long (- (* long 2) 2))
                   (setf next
                           (append half (cdr (reverse (cdr half))))))
                  ((member mode '(nil 0) :test #'eq)
                   (setf long (* long 2))
                   (setf next (append half (reverse half))))
                  ((member mode
                           '(-1 :first first :start start :left left)
                           :test #'eq)
                   (setf long (- (* long 2) 1))
                   (setf next (append half (reverse (cdr half)))))
                  ((member mode '(1 :last last :end end :right right)
                           :test #'eq)
                   (setf long (- (* long 2) 1))
                   (setf next (append half (cdr (reverse half)))))
                  (t
                   (error
                    "~s is not an elide value: ~s, ~s, :first, :last."
                    mode true false)))
...
(defmethod initialize-instance :after ((obj line) &rest args) args
           (let ((cyc (make-cycl)))
             (cycl-data-set! cyc (pattern-data obj))
             (cycl-tail-set! cyc (cycl-data cyc))
             (setf (pattern-data obj) cyc)
...
(defmethod next-in-pattern ((obj line))
           (let ((cycl (pattern-data obj)))
             (if (null (cdr (cycl-tail cycl))) (car (cycl-tail cycl))
                 (pop-cycl cycl))))
...
(defmethod pattern-external-inits ((obj heap))
           (let ((inits (call-next-method)))
             (if (eq *random-state* (pattern-random-state obj)) inits
                 (append inits
                         (list :state (pattern-random-state obj))))))

(defmethod initialize-instance :after ((obj heap) &rest args) args
           (let ((cyc (pattern-data obj)))
...
(defmethod next-in-pattern ((obj heap))
           (flet ((shufl (lis len state)
                    (loop for i below len for j = (random len state)
                          for v = (elt lis i) do
           ...
           (flet ((fnc (n)
                    (let ((dat
                      ...
                      (unless (null (random-item-max n))
                        (push
                         (expand-pattern-value (random-item-max n))
                         lst)
                        (push ':max lst))
                      (unless
                          (or (not (numberp (random-item-min n)))
                              (= (random-item-min n) 1))
                        (push
                         (expand-pattern-value (random-item-min n))
                         lst)
                        (push ':min lst))
                      (unless
                          (and (numberp (random-item-weight n))
                               (= (random-item-weight n) 1))
                        (push
                         (expand-pattern-value
                          (random-item-weight n))
                         lst)
                        (push ':weight lst))
                      (if (null lst) dat
...
(defmethod default-period-length ((obj weighting))
           (if (weighting-adjustable obj) 1
               (do ((tail (pattern-data obj) (cdr tail))
                 ...
                 (setf flag
                         (pattern? (random-item-datum (car tail)))))))
...
(defmethod initialize-instance :after ((obj weighting) &rest args)
           ...
           (let ((pool (pattern-data obj))
                 ...
                 (const t))
             (loop for item in pool for min = (random-item-min item)
                   for max = (random-item-max item) unless
                   (and (or (not min) (numberp min))
             ...
             (when const
               (setf (pattern-flags obj)
                       (logior +constant-minmax+
                               (pattern-flags obj))))
             (loop for item in pool for weight =
                   (random-item-weight item) while sum if
                   (numberp weight) do
                   (progn
                    (incf sum weight)
                    (random-item-index-set! item sum))
                   else do (setf sum nil) finally
                   (when (and sum *random-range*)
                     (dolist (item pool)
                       (random-item-index-set! item
...
(defmethod canonicalize-pattern-data
           ((obj weighting) data parser inits) obj inits
           (flet ((parse-random-item (extern)
                    (apply
                     (lambda (datum &rest keys)
                       (setf datum (maybeparse parser datum))
                       (loop with orig = keys and args = '() and key
                             and val while (not (null keys)) do
                             (setf key (pop keys))
                             (setf val
                                     (if keys (pop keys)
                                         (error
...
(defun reset-random-range (obj range adj?)
  (let ((data (car (pattern-data obj))))
    (loop with s = (if (integerp range) 0 0.0) for item in data do
          (incf s
                (if adj? (random-item-weight item)
                    (next-1 (random-item-weight item))))
          (random-item-index-set! item s) finally
          (if range
              (dolist (i data)
           ...
           (let ((reset (call-next-method))
             ...
             (unless (logtest flags +constant-weights+)
               (loop with s = (if (integerp *random-range*) 0 0.0)
                     for item in (car (pattern-data obj)) do
                ...
                (setf next
                        (loop for item =
                              (loop with index = (random range state)
                                    for x in choices when
                                    (< index (random-item-index x))
                                    return x)
                              unless
                              (and (random-item-max item)
                                   (= (random-item-count item)
                                      (random-item-max item)))
                              return item))
                (unless (eq next last)
                  (dolist (i choices) (random-item-count-set! i 0)))
                (rplacd pool next)
                (when adjust
                  (random-item-weight-set! next
                   (* (random-item-weight next) adjust)))
                (random-item-datum next)))))

(defmethod map-pattern-data (fn (obj weighting))
           (map nil (lambda (x) (funcall fn (random-item-datum x)))
...
(defmethod pattern-external-inits ((obj markov))
           (flet ((fnc (n)
                    `',(append (first n) (list '->)
                               (loop with s = 0 for x in (cddr n) for
                                     w = (- (second x) s) collect
                                     (if (= w 1) (car x)
                                         (list (car x) w))
                                     do (setf s (second x))))))
             (append
              (list ':of
                    (cons 'list
                          (loop for x in (cdr (pattern-data obj))
                                collect (fnc x))))
              (call-next-method)
              (if (not (markov-pattern-produce obj)) (list)
                  (list ':produce
                        (expand-pattern-value
           ...
           (flet ((parse-markov-spec (spec)
                    (let ((tail
                      ...
                      (if tail
                          (progn
                           (setf inputs
                                   (loop until (eq spec tail) collect
                                         (pop spec)))
                           (setf parse (cdr tail)))
                          (progn
                           (setf inputs '())
                        ...
                        (let ((val nil) (pat nil) (wei nil))
                          (if (consp s)
                              ...
                              (progn
                               ...
                               (setf wei 1)
                               (setf pat 1)))
                          (if (and wei range) (incf range wei)
                              (setf range nil))
             ...
             (let ((const t))
               ...
               (let ((coll (list nil)))
                 (do ((tail data (cdr tail))
                   ...
                   (if (not order) (setf order (length (first p)))
                       (setf order (max order (length (first p)))))
                   (rplacd lis (list p))
                   (setf lis (cdr lis)))))))

(defmethod initialize-instance :after ((obj markov) &rest args) args
           ...
           (labels ((select-output (range outputs)
                      (if (not range)
                          (do ((tail outputs (cdr tail))
                            ...
                            (rplaca (cdr out) sum))
                          (let ((n (random range)))
                            (loop for o in outputs when
                                  (< n (second o)) return
                                  (first o)))))
                    (match-past (inputs past)
                      (loop for i in inputs for j in past unless
                            (or (eq i '*) (equal i j) (eq j '*))
                            return nil finally (return t))))
             (let ((past (markov-pattern-past obj))
                   (data (markov-pattern-produce obj))
                   (item nil))
               (loop for i in (pattern-data obj) when
                     (or (null past) (match-past (first i) past)) do
                     (return
                      (let ((last nil))
                        (setf item
                                (select-output (second i) (cddr i)))
                        (unless (null past)
                          (if (null (cdr past)) (rplaca past item)
                              (progn
                               (setf last (last past))
                               (rplaca past item)
                               (rplacd last past)
                     ...
                     (error "No outputs for past choices ~s."
...
(defun default-graph-node-select (obj node lastids)
  (let ((to (graph-node-to node)))
    (if (consp to)
        (if (eq (car to) ':idsel) (markov-select obj node to lastids)
...
(progn
 (defclass graph (pattern)
           ((selector :initform #'default-graph-node-select :initarg
            ...
            (props :initform '() :initarg :props :accessor
             graph-props)
           ...
           (flet ((fnc (n)
                    (cons 'list
                          (list
                           (expand-pattern-value
                            (graph-node-datum n))
                           ':id (graph-node-id n) ':to
                           (expand-pattern-value
                            (graph-node-to n))))))
             (append
              (list ':of
                    (cons 'list
                          (loop for x in (pattern-data obj) collect
                                (fnc x))))
              (call-next-method)
              (if (null (graph-props obj)) (list)
                  (list ':props
                        (expand-pattern-value (graph-props obj))))
              (if
               (eq (graph-selector obj) #'default-graph-node-select)
               (list) (list ':selector (graph-selector obj))))))

(defmethod initialize-instance :after ((obj graph) &rest args) args
           (let ((nodes (pattern-data obj)) (last (graph-last obj)))
             (when last
               (setf (graph-last obj)
                       (if (consp last) (cons (length last) last)
                           (cons last
                                 (make-list last :initial-element
                                            '*)))))
             (setf (pattern-data obj)
                     (cons
                      (elt nodes (graph-starting-node-index obj))
                      nodes)))
           (values))
           ...
           (flet ((parse-graph-item (extern)
                       ...
                       (do ((orig keys)
                           ...
                           ((null keys)
                            (unless id
                              (push datum args)
                              (push ':id args))
                            (apply #'make-graph-node :datum
                                   (maybeparse parser datum) args))
                         (setf key (pop keys))
                         (setf val
                                 (if (null keys)
                                     (error "Bad graph node: ~s."
                                            orig)
                                     (pop keys)))
                         (push val args)
                         (case key
                           ...
                           (t
                            (error "~s not one of: :id, :to, :props."
                                   key)))))
                     extern)))
             (let ((intern (mapcar #'parse-graph-item data)))
               (values intern (length intern)
                       (not
                        (some
                         (lambda (x) (pattern? (graph-node-datum x)))
                         intern))))))

(defun markov-select (obj node table lastids)
  obj
  (let ((prob
         (loop for tail on (cdr table) by #'cddr when
               (match-ids (car tail) lastids) return (cadr tail))))
    (unless prob
...
(defun match-ids (user last)
  (cond ((null user) t) ((null last) nil)
        ((consp user)
         (and (match-ids (car user) (car last))
              (match-ids (cdr user) (cdr last))))
        ((eq user last) t) ((eq user '*) t) ((eq last '*) t) (t nil)))
...
(defmethod next-in-pattern ((obj graph))
           (let* ((last (graph-last obj))
                  ...
                  (next nil))
             (setf next
                     (funcall (graph-selector obj) obj this
                              (if last (cdr last))))
             (if next
                 (let ((node
                        (find-if
                         (lambda (x) (eq next (graph-node-id x)))
                         nodes)))
                   (if node
                       (progn
                        (rplaca graph node)
                        (when last
                          (rplacd last
                                  (cons (graph-node-id this)
                                        (cdr last)))
                          (rplacd
                           (nthcdr (- (car last) 1) (cdr last))
                           '())))
                       (error "No node for id ~s." next)))
                 (error "No next node from ~s."
                        (graph-node-id this)))
             (graph-node-datum this)))

(defmethod map-pattern-data (fn (obj graph))
           (map nil (lambda (x) (funcall fn (graph-node-datum x)))
                (pattern-data obj)))
...
(defmethod initialize-instance :after ((obj thunk) &rest args) args
           ...
           (append
            (list ':of
                  (cons 'list
                        (loop for x in (car (pattern-data obj))
                              collect (expand-pattern-value x))))
            (if (equal (rotation-change obj) 0) (list)
                (list ':rotations
           ...
           (let ((ring (pattern-data obj)))
             (when (null (cdr ring))
               (let ((change (next-1 (rotation-change obj)))
                 ...
                 (unless end
                   (setf end (- (pattern-length obj) width)))
                 (rplacd ring
                         (rotate-items (car ring) start end step
                          width))))
             (pop (cdr ring))))

(defun rotate-items (items start end step width)
  (loop for i from start below end by step for a = (elt items i) for
        b = (elt items (+ i width)) do (setf (elt items i) b)
           ...
           (flet ((fnc (n)
                    (cons 'list
                          (append
                           ...
                           (if
                            (eq (rewrite-node-datum n)
                                (rewrite-node-id n))
                            (list) (list ':id (rewrite-node-id n)))
                           (list ':to
                                 `',(loop for x in
                                          (rewrite-node-to n) collect
                                          (if (listp x)
                                              (rewrite-node-id x)
                                              x)))))))
             (append
...
(defmethod initialize-instance :after ((obj rewrite) &rest args) args
...
(defmethod canonicalize-pattern-data
           ((obj rewrite) data parser inits) obj inits
                    ...
                    (let ((datum nil) (keys '()))
                      ...
                      (loop with args = '() and id and key and val
                            ...
                            (case key
                              ...
                              (t
                               (error
                                "~s not one of rewrite keywords: id ->."
                                key)))
                            finally
                            (progn
                             (unless id
...
(defmethod map-pattern-data (fn (obj rewrite))
           (map nil (lambda (x) (funcall fn (rewrite-node-datum x)))
                (car (pattern-data obj))))
...
(defun parse-rules (rules table)
  (labels ((getnode (id table rule)
             (or (gethash id table)
                 (if (eq id '*) '*
                     (error "No node for id ~s in rule ~s." id
                            rule))))
           (getnodes (ids table rule)
             (loop for id in ids collect (getnode id table rule))))
    (loop for rule in rules collect
          (loop with form = rule and left = t and x while
                (not (null form)) do (setf x (pop form)) if
                (or (eq x '->) (eq x ':->)) do (setf left nil) else
                if left collect x into lh else collect x into rh
                finally
                (return
                 (let ((len (length lh)))
                   (unless (not left)
                   ...
                   (cond
                    ...
                    ((= len 1)
                     (make-rewrite-rule :successors
                      (getnodes rh table rule) :trigger
                      (getnode
                       (if (consp (car lh)) (caar lh) (car lh)) table
                       rule)))
                    (t
                     (loop for tail on lh for x = (car tail) for i
                           from 0 when (consp x) do
                           (rplaca tail (car x))
                           (return
                            (make-rewrite-rule :successors
                             (getnodes rh table rule) :trigger
                             (getnode (car x) table rule) :context
                             (cons (cons i len)
                                   (getnodes lh table rule))))
                           finally
                           (error "No trigger in lh side of ~s."
                                  rule))))))))))

(defun lookup-successors (successor table)
  (if (consp successor)
      (loop for to in successor collect
            (or (gethash to table)
                (error "No rewrite node for id ~s." to)))
      (if successor
          (list
           (or (gethash successor table)
  ...
  (let ((data (pattern-data obj)))
    (if (not new)
        ...
        (let* ((nodes (pattern-data obj))
          ...
          (when (null new)
            (error "Rewrite generation #~s is empty!"
                   (+ (car (rewrite-generations obj)) 1)))
          (progn (rplacd nodes new) (rplaca nodes new))
          (if (not ids) (car nodes)
              (mapcar #'rewrite-node-datum (car data)))))))

(defun node-rewrite (gen table)
  (loop for node in gen for next =
        (let ((to (rewrite-node-to node)))
          (if to (copy-list to)
              (lookup-successors (next-1 (rewrite-node-props node))
               table)))
        nconc next))
  ...
  (let ((start1 0) (end1 (length seq)) (start2 beg) (end2 end))
    (do ((i1 start1 (+ 1 i1))
        ...
        ((or done (and (>= i1 end1) (>= i2 end2)))
         (if done done nil))
      (if (>= i1 end1) (setf done i1)
          (if (>= i2 end2) (setf done i1)
              (progn
               (setf x1 (elt seq i1))
               (setf x2 (elt gen i2))
...
(defun rule-rewrite (generation rules)
  (loop with context and len for index from 0 for node in generation
        append
...
(progn
 (defclass range (pattern)
           ((from :initform nil :initarg :from :initarg :initially
            ...
            (by :initform nil :initarg :by :initarg :stepping
             :accessor range-by)
...
(defmethod pattern-external-inits ((obj range))
           (let ((bits (pattern-flags obj)))
             (append
              (list (if (%range-initially? bits) ':initially ':from)
                    (expand-pattern-value (range-from obj)))
              (if (range-to obj)
                  (list
                   (if (%range-random? bits) ':pickto
                       (if (funcall (range-test obj) 2 0 2) ':below
                           ':to))
                   (expand-pattern-value (range-to obj)))
                  (if (range-downto obj)
                      (list
                       (if (funcall (range-test obj) 0 0 2) ':above
                           :downto)
...
(defmethod initialize-instance :after ((obj range) &rest args) args
...
(defmethod canonicalize-pattern-data ((obj range) data parser inits)
           ...
           (let ((from 0)
             ...
             (if
              (or (pattern? from) (pattern? to) (pattern? downto)
                  (pattern? by))
              (setf const? nil))
             (values
              (if const?
                  ...
                  (list (list nil nil nil nil nil)
                        (list from to downto by incf flag bits)))
              (if (or to downto) most-positive-fixnum 1) const?)))
           ...
           (let ((bits (pattern-flags obj))
             ...
             (when (reset-range? data test)
               (if (logtest bits +constant-data+)
                   (progn
                    (unless
                        (and (%range-initially? bits) (third data))
             ...
             (if (%range-random? bits)
                 (progn
                  (setf from
                          (funcall (range-incf obj) (second data)
                                   (car (cddddr data))))
                  (rplaca (cddr data) from))
                 (progn
                  (setf from (third data))
                  (rplaca (cddr data)
                          (funcall (range-incf obj) from
                                   (if (%range-stepping? bits)
                                       (next-1 (range-by obj))
                                       (first data))))))
             (if
              (and (%range-dynamic? bits) (reset-range? data test))
...
(progn
 (defclass transposer (container)
           ((of :initarg :of :accessor transposer-of)
            (by :accessor transposer-by :initarg :by :initarg
             :stepping :initarg :on)
            (form :accessor transposer-form :initform nil :initarg
             :form)
            (mod :accessor transposer-mod :initform nil :initarg
                 :mod)
                                    ...
                                    (list ':by
                                          (if (second by)
                                              (expand-pattern-value
                                               (second by))
                                              (first by)))))
                              (if (not (transposer-form obj)) (list)
                                  (list ':form
                                        (expand-pattern-value
                                         (transposer-form obj))))
                              (if (not (transposer-mod obj)) (list)
                                  (list ':mod
                                        (expand-pattern-value
                                         (transposer-mod obj))))
             ...
             (dopairs (s v args)
              (case s
                ...
                ((:by :on)
                 ...
                 (setf data v))))
             (unless data
               (error "Missing ':stepping', ':by' or ':on' in: ~s"
                      args))
             (setf (transposer-by obj)
                     (if stepping? (list nil data t)
                         (if (pattern? data) (list nil data nil)
                             (list data nil nil))))
             (unless (slot-boundp obj 'of)
               (error "Missing ':of' data."))
             (setf data (transposer-form obj))
             (when data
               (setf (transposer-form obj)
                       (if (and data (symbolp data)) (list data nil)
                           (list nil data))))))
...
(defmethod next-1 ((obj transposer))
           (let* ((by (transposer-by obj))
             ...
             (when (transposer-mod obj)
               (setf data
                       (scale-mod data (transposer-mod obj) :offset
                        (first data) :in (transposer-scale obj))))
...
(defmethod default-period-length ((obj chord)) obj 1)

(defmethod next-in-pattern ((obj chord))
           (let ((data (pattern-data obj)))
             (if (logtest (pattern-flags obj) +constant-data+) data
                 (if (null (cdr data)) (next (first data) t)
                     (apply #'append
...
(progn
 (defclass join (pattern)
           ((format :accessor join-format :initarg :format :initform
                    nil)
...
(defmethod initialize-instance :after ((obj join) &rest args) args
           (let* ((per (pattern-period obj))
             ...
             (cond
              ...
              ((not (listp fmat))
               (error "Expected :format list but got ~s instead."
                      fmat))
              ((not (= (length fmat) len))
               (error
             ...
             (if parser
                 (let ((coll (list nil)))
                   (do ((tail data (cdr tail))
                     ...
                     (if (pattern? (car tail)) (setf num (+ num 1)))
                     (rplacd lst
                             (list (maybeparse parser (car tail))))
                     (setf lst (cdr lst))))
                 (values data (if subs 1 most-positive-fixnum)
                         (not subs)))))
...
(defmethod next-in-pattern ((obj join))
           (let ((next (list nil))
             ...
             (do ((l1 data (cdr l1))
                  ...
                  (l3 next (cdr l3)))
                 ((null l1) nil)
               (if (not (car l2)) (rplacd l3 (list (car l1)))
...
(defmethod pattern-external-inits ((obj copier))
           (append
            (list ':of (expand-pattern-value (copier-source obj)))
            (call-next-method)))

(defmethod initialize-instance :after ((obj copier) &rest args) args
           (let ((data (pattern-data obj)))
           ...
           (let ((data (pattern-data obj)))
             (if (null data)
                 (let* ((per (pattern-period obj))
                   ...
                   (car res))
                 (progn
                  (setf (pattern-data obj) (cdr data))
                  (car data)))))

(defparameter pattern-types
  '(cycle heap weighting line palindrome graph markov rewrite range
    rotation))

(defparameter pattern-item-types
  '(notes keynums rhythms amplitudes hertz))

(defparameter pattern-options
  '((pattern alias with) (palindrome elide)
    (of notes keynums hertz rhythms amplitudes) (notes in through)
    (keynums in through from) (hertz in through) (rhythms tempo)
    (amplitudes loudest softest) (heap state)
    (weighting state adjustable) (markov past produce)
    (graph last selector props starting-node-index)
    (rotation rotations) (rewrite initially rules generations)
    (rotation rotations)
    (range from initially to below pickto downto above by stepping)
    repeat eop-hook for name))

(defparameter pattern-no-items '(range transposer))

(defun gather-substs (forms)
  (flet ((subst-clause? (form last)
           (and
            (or (member (car form) '(alias with))
        ...
        (setf subst
                (cons
                 (make-pattern-binding
                  (if (eq (first tail) 'and) last (first tail)) tail)
                 subst))
        (if (not (eq (car tail) 'and)) (setf last (car tail)))))))

(defun make-pattern-binding (typ args)
  (if (not typ) (error "Extraneous 'and' found: ~s" args)
          ...
          (if
           ...
           (if (eq (third args) '=)
               (let ((var (second args)) (val (fourth args)))
                 (if (numberp val) (list var val)
                     (if (eq typ 'with) (list var val)
                         (list var `(lambda () ,val)))))
               (error "Expected '=' but got '~s' instead: ~s"
                      (third args) args))
           (error
            "Expected variable name (symbol) but got '~s' instead: ~s"
            (second args) args)))))

(defun check-pattern-option (form option err?)
  (cond
   ((null form)
    (if err? (error "Pattern ended without expected token.") nil))
   ((consp option)
    (if (member (car form) option) t
        (if err?
            (error
             "Expected pattern option from ~S but got ~S instead."
             option form)
            nil)))
   (t
    (if (eq (car form) option) t
        (if err?
            (error "Expected pattern option ~S but got ~S instead."
                   option form)
            nil)))))
        ...
        ((consp item)
         (if (not (member ptyp '(weighting markov graph rewrite)))
             ...
             `(cons ,(expand-pattern-item (car item) nil etyp vars)
                    ,(expand-pattern-item (cdr item) ptyp etyp
                      vars))))
        ((keyword? item) item)
        ((and item (symbolp item))
         (if (assoc item vars) item `',item))
        ((numberp item) item) (t `',item)))
...
(defun gather-pattern-data (args num stop ptyp etyp vars)
  ...
  (do ((tail args)
      ...
      ((or (null tail) done)
       (cond
        ((null args) (error "Pattern ended without pattern data."))
        ((or (and (numberp num) (< nget num))
             (and (eq num t) (null gets)))
         (error "Expected pattern data but got ~S instead." args))
        (t (values (cons 'list (nreverse gets)) tail))))
    (if
     (or (member (car tail) stop) (and (numberp num) (= nget num)))
     (setf done t)
...
(defun expand-pattern-macro (form)
  (let ((vars (list))
    ...
    (if (null vars) form (list 'let* vars form))))
