...
;;; generated by scheme->cltl from scales.scm on 30-Jan-2007 07:38:52
...
(progn
 (defclass scale (container)
           ((octave :accessor scale-octave :initarg :octave :initform
             nil)
...
(progn
 (defclass tuning (scale)
           ((steps :initform '() :accessor scale-steps :initarg
             :steps :initarg :cents :initarg :ratios)
            (table :accessor scale-table :initform 10 :initarg
             :octaves)
            (defoct :initform 4 :initarg :default-octave :accessor
             scale-default-octave)
            (octave-offset :initform 0 :accessor
             scale-octave-offset)))
...
(progn
 (defclass mode (scale)
           ((steps :initform '() :accessor scale-steps :initarg
             :steps :initarg :degrees :initarg :notes)
            (lowest :accessor scale-lowest :initform nil :initarg
             :tonic)
...
(defmethod initialize-instance :after ((obj tuning) &rest args) args
           (let ((extern nil)
                 (steps? t)
                 (cents? t)
                 (equal? nil)
                ...
                ((:steps)
                 (if extern
                     (error
                      "only one of :cents, :ratios or :steps allowed."))
                 (setf cents? nil)
                 (setf octave (or (scale-octave obj) 2.0))
                 (unless (and (numberp v) (integerp v))
                   (error
                    "tuning: :steps value ~s is not an integer." v))
                 (setf extern
                         (loop for i to v collect
                               (expt octave (/ i v))))
                 (print extern))))
             (unless (consp extern)
               (error
             ...
             (cond
              ((consp (car extern))
               (setf nums
                       (loop for e in extern collect
                             (if (and (consp e) (numberp (car e)))
                                 (car e)
                                 (if (numberp e) e
                                     (error "Bad degree: ~s in ~s." e
                                            extern)))))
               (setf steps?
                       (not
                        (if cents? (= (car nums) 0)
                            (= (car nums) 1))))
               (when
                   (and (not steps?) (consp (car (last extern)))
                        (scale-table obj))
                 (error "No ending octave size (cent or ratio): ~s "
                        extern))
               (setf syms
                       (loop for e in extern while (consp e) collect
                             (if (null (cdr e))
                                 (error
                                  "Degree spec missing notes: ~s." e)
                                 (cdr e)))))
              ((numberp (car extern)) (setf nums extern)
               (setf steps?
                       (not
                        (if cents? (= (car nums) 0)
                            (= (car nums) 1))))
                     ...
                     (if steps? (apply #'= nums)
                         (let ((one
                                (if cents?
                                    ...
                                    (/ (second nums) (first nums)))))
                           (loop for tail on nums while
                                 (not (null (cdr tail))) always
                                 (let ((a (first tail))
                                       (b (second tail)))
                                   (= one
                                      (if cents? (- b a)
             ...
             (if equal?
                 (if steps?
                     (progn
                      (setf data
                              (if cents? (cents->scaler (first nums))
                                  (first nums)))
                      (setf octave
                              (if cents?
                 ...
                 (progn
                  ...
                  (setf octave
                          (if cents? (cents->scaler (pop data))
                              (pop data)))
                  (setf data
                          (if cents?
                              (mapcar #'cents->scaler (reverse data))
                              (reverse data)))
                  (setf len (length data))
                  (setf (scale-into obj)
                          (loop for s in data collect
                                (log s octave)))))
             (setf (scale-steps obj) data)
             (setf (scale-divisions obj) len)
             (if (not syms) (setf (scale-table obj) nil)
                 (let ((octaves (scale-table obj))
                       (table
                        (make-hash-table :size 103 :test #'equal))
                       (degrees
                        (loop for deg in syms for i from 0 collect
                              (loop for d in deg unless (consp d) do
                                    (setf d (list d)) collect
                                    (let ((n (pop d)))
                                      (make-sd :note n :keynum i
                                       :octave (getf d :octave)
                                       :class i :name n :accidental
                                       (getf d :accidental)))))))
             ...
             (if (scale-lowest obj)
                 (unless (numberp (scale-lowest obj))
                   (error "lowest not hertz value: ~s."
                          (scale-lowest obj)))
                 (setf (scale-lowest obj) (* 6.875 (expt 2 1/4))))))
...
(defun fill-tuning-table (tuning degrees octaves)
  ...
  (let ((table (scale-table tuning))
    ...
    (if (consp octaves)
        (progn
         (setf begoct (first octaves))
         (setf endoct (second octaves)))
        (if octaves
            (progn (setf begoct 0) (setf endoct (- octaves 1)))
    ...
    (do ((num begoct (+ num 1))
      ...
      (do ((lst degrees (cdr lst))
        ...
        (let* ((k (+ (* oct divisions) i))
               (l
                (loop for d in deg for n = (sd-note d) for o =
                      (+ num (or (sd-octave d) 0)) collect
                      (make-sd :note
                       (if octaves
                           (intern
                            (string-upcase (format nil "~A~A" n o))
                            :cm)
                           (sd-note d))
          ...
          (dolist (x l)
            (setf (gethash (sd-note x) table)
                    (cons x (remove x l))))))))
  (when octaves
    (dolist (degs degrees) (dolist (d degs) (sd-octave-set! d nil)))))

(defun scale-ref (table ref)
  (or (gethash ref table)
      (if (and ref (symbolp ref))
          (gethash (intern (string-upcase (symbol-name ref)) :cm)
                   table)
          nil)))

(defun equal-tuning? (tuning)
  (let ((steps (scale-steps tuning)))
    (if (consp steps) nil steps)))

(defmethod mode-tuning ((obj scale)) obj nil)
...
(defun octave-equivalent
       (note &optional (octave 0) (scale *chromatic-scale*))
  (let* ((table (scale-table scale))
    ...
    (if entry
        (loop for e in
              ...
              when (eq (sd-name e) (sd-name entry)) return e)
           ...
           (mapcar
            (lambda (n)
              (or (note-name n tuning)
                  (error "Note '~s' not defined in ~s." n tuning)))
            notes)
...
(defmethod initialize-instance :after ((obj mode) &rest args) args
           (let* ((spec nil)
             ...
             (dopairs (a v args)
              (case a
                ...
                ((:degrees :notes)
                 (when spec
                   (error "Found duplicate keywords ~s and ~s." type
                          a))
                 (setf spec v)
                 (setf type a))
                (t nil)))
             ...
             (let ((into
                    (make-list (interval-semitones octave)
                               :initial-element nil)))
               (loop for step in steps for degree from 0 do
                     (setf (elt into (interval-semitones step))
                             degree))
           ...
           (let ((div (interval-semitones (scale-octave mode)))
             ...
             (multiple-value-bind
               ...
               (setf deg
                       (or (elt into int)
                           (if force?
                               (loop for i from int downto 0 for j =
                                     (elt into i) when j return j)
                               nil)))
             ...
             (multiple-value-bind
               ...
               (if (and (eq return ':note) (%interval-encoded? int))
                   (do ((low (car (scale-lowest mode)))
                     ...
                     (setf low
                             (transpose low (scale-octave mode)
                              scale)))
                   (let ((num
                          (+
                           (* oct
...
(defmethod tuning-hertz->keynum ((obj tuning) hz)
           (let ((low (scale-lowest obj))
             ...
             (if int (- (* (- (log hz oct) (log low oct)) div) off)
                   ...
                   (do ((i 0 (+ i 1))
                     ...
                     (if (<= x octrem y)
                         (if (< (- y octrem) (- octrem x))
                             (setf int (+ 1 i)) (setf int i))
                         (if (null (cdr into)) (setf int (+ 1 i))
                             (setf into (cdr into)))))))))
...
(defmethod tuning-keynum->note ((obj tuning) knum acci err?)
           (let* ((table (scale-table obj))
                  (entries
                   (and table (scale-ref table (round knum)))))
             (if entries
                 (let ((it
                        (if acci
                            (if (consp acci)
                                (loop for a in acci for x =
                                      (find-if
                                       (lambda (x)
                   ...
                   (if it (sd-note it)
                       (if err?
                           (error
                            "No note with accidental ~s for keynum ~s in ~s."
                            acci (round knum) obj)
                           nil)))
                 (if err?
                     (if table
                         (error "No note for keynum ~s in ~s." knum
                                obj)
                         (error "No note table defined in ~s." obj))
                     nil))))

(defparameter %oct nil)
...
(defmethod tuning-note->note ((obj tuning) note acci err?)
           (if acci
               (tuning-keynum->note obj
                (tuning-note->keynum obj note err?) acci err?)
               (let* ((table (scale-table obj))
                      (entries (and table (scale-ref table note))))
                 (if entries
                     (tuning-keynum->note obj
                     ...
                     (if err?
                         (if table
                             ...
                             (error "No note table defined in ~s."
                                    obj))
                         nil)))))

(defparameter %hertz '(:hertz hertz :hz hz))
...
(defmacro with-default-octave (scale &body body)
  (let ((v (gensym "v")))
    `(let ((,v nil))
       (unwind-protect
           (progn
            (setf %oct (or %oct (scale-default-octave ,scale)))
            (setf ,v ,@body))
         (setf %oct nil))
       ,v)))
  ...
  (let ((hz? nil) (scale nil) (mode nil) (tuning nil) (oper nil))
    (unless freq
      (error
       ...
       false))
    (when
        (and (consp args) (member (first args) %hertz)
             (oddp (length args)))
    ...
    (if (consp freq)
        ...
        (if (and (not mode) hz?) freq
            (let ((key
                   (if hz? (tuning-hertz->keynum tuning freq)
...
(defun keynum (freq &rest args)
  (let ((hz? nil)
    ...
    (unless scale (setf scale *scale*))
    (if (typep scale <mode>)
        (progn (setf tuning (mode-tuning scale)) (setf mode scale))
        (progn (setf tuning scale)))
    (if (consp freq)
        (with-default-octave tuning
              ...
              (if (eq oper ':from)
                  (if (integerp key)
                      ...
                      (error "keynum:  :from value ~s not integer."
                             key))
                  (if (eq oper ':through) (values (round key))
                      key)))))))
...
(defun note (freq &rest args)
  (let ((oper nil)
    ...
    (when (and (eq oper ':from) (or hz? (and freq (symbolp freq))))
      (error ":from conversion: ~s is not a key number in ~s." freq
             mode))
    (cond
     ((consp freq)
      (with-default-octave tuning
...
(defmethod scale-mod
           ((freq number) modulus &key (offset t) (in *scale*)
            accidental)
           accidental
...
(defmethod scale-mod
           ((freq symbol) modulus &key (offset t) (in *scale*)
            accidental)
           (tuning-keynum->note in
            (scale-mod (keynum freq) modulus :offset offset :in in)
            accidental t))

(defmethod scale-mod
           ((freq cons) modulus &key (offset t) (in *scale*)
            accidental)
           (if (eq offset t) (setf offset (first freq)))
           (loop for f in freq collect
                 (scale-mod f modulus :in in :offset offset
...
(defmethod transpose ((note symbol) int &optional (scale *scale*))
           (if (eq note 'r) note
               (if (numberp int)
                   (let* ((table (scale-table scale))
                          (entries
                           (and table (scale-ref table note)))
                          (entry (and entries (first entries))))
                     (if entry
                         (if (sd-octave entry)
                             (if %oct (setf %oct (sd-octave entry)))
                                     ...
                                     (octave-equivalent note
                                      (or %oct
                                          (scale-default-octave
                                           scale))
                                      scale)))
                         (error "~s is not a note in ~s." note
                                scale))
                     (if
                      (and (eq scale *chromatic-scale*)
                           (integerp int) (%interval-encoded? int))
                      (let* ((names
                              '((cff cf c cn cs css)
                                (dff df d dn ds dss)
                                (eff ef e en es ess)
                             ...
                             (entries
                              (scale-ref table
                               (if (sd-octave entry) keyn
                                   (mod keyn 12))))
                             (newnote
                              (or
                               (loop for n in trans for e =
                                     (find-if
                                      (lambda (x) (eq n (sd-name x)))
                                      entries)
                                     when e return e)
                               (error "Can't transpose ~a by ~a."
                                      note (decode-interval int)))))
                        (if (sd-octave entry) (sd-note newnote)
                            (sd-name newnote)))
                       ...
                       (let* ((sign1 (%interval-sign note))
                         ...
                         (setf type
                                 (elt types
                                      (mod (- steps offset) 12)))
                         (encode-interval
                          (first (elt interval-names type))
                          (+ width 1)))
                       (+ (interval-semitones note) int))
                   (+ note
                      (if
                       (and (integerp int) (%interval-encoded? int))
                       (interval-semitones int) int)))
               (if (and int (symbolp int)) (transpose int note scale)
                   (error "Don't know how to transpose ~s by ~s."
                          note int))))
...
(defmethod invert ((lst cons) &optional pc?)
           (let* ((n (car lst)) (k (keynum n)))
             (loop for x in lst for d = (- k (keynum x)) collect
                   (transpose n (if pc? (mod d 12) d)))))

(defmethod interval ((ref cons) &optional ref2) ref2
...
(defmethod interval ((ref integer) &optional ref2)
           (if (null ref2) ref
               (let* ((steps ref)
                 ...
                 (setf type
                         (elt types (mod (- (abs steps) offset) 12)))
                 (setf int
                         (+ (* (floor (/ (abs steps) 12)) 7) width))
                 (encode-interval (first (elt interval-names type))
                  (if (< steps 0) (- int) int)))))

(defmethod interval ((ref symbol) &optional ref2)
           (cond
            ((note-in-scale? ref *chromatic-scale*)
             (let* ((table (scale-table *chromatic-scale*))
               ...
               (if (not entry1)
                   (error "~s not in standard chromatic scale."
                          ref2))
               (setf letters
                       (mod
                        (-
                         (position-if
                          (lambda (x) (member (sd-name entry1) x))
                          names)
                         (position-if
                          (lambda (x) (member (sd-name entry2) x))
                          names))
                        7))
               (setf semitones
                       (- (sd-keynum entry1) (sd-keynum entry2)))
...
(setf *chromatic-scale*
        (make-instance <tuning> :name "chromatic-scale" :octaves
                       ...
                       '((100 (c) (cn :accidental n)
                         ...
                         (100 (d) (dn :accidental n)
                          (css :accidental ss) (eff :accidental ff))
                         (100 (ef :accidental f)
                          (ds :letter d :accidental s)
                          (fff :letter f :accidental ff))
                         (100 (e) (en :accidental n)
                          (ff :accidental f) (dss :accidental ss))
                         (100 (f) (fn :accidental n)
                          (es :accidental s) (gff :accidental ff))
                         (100 (fs :accidental s) (gf :accidental f)
                          (ess :accidental ss))
                         (100 (g) (gn :accidental n)
                          (fss :accidental ss) (aff :accidental ff))
                         (100 (af :accidental f) (gs :accidental s))
                         (100 (a) (an :accidental n)
                          (gss :accidental ss) (bff :accidental ff))
