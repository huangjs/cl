...
;;; generated by scheme->cltl from sco.scm on 30-Jan-2007 07:38:58
...
(defun make-sco-writer (objclassname objclassvar pars supers sdecl)
  sdecl
  supers
  (define-output-method objclassname objclassvar 'obj 'sco-file
   '<sco-file> 'io 'scoretime
   (list
    `(let ((fp (io-open io)))
       (princ (sco-name obj) fp)
       ,@(mapcar (lambda (p) (sco-par-print p 'obj 'fp 'scoretime))
                 pars)
       (terpri fp)
       (values)))))

(progn
 (defclass sco-file (event-file)
           ((header :initform nil :initarg :header :accessor
             sco-file-header))
           #+metaclasses  (:metaclass io-class))
 (defparameter <sco-file> (find-class 'sco-file))
 (finalize-class <sco-file>)
 (setf (io-class-definer <sco-file>) #'make-sco-writer)
 (setf (io-class-file-types <sco-file>) '("*.sco"))
 (values))

(defun set-sco-output-hook! (fn)
  (unless (or (not fn) (functionp fn))
    (error "Not a sco hook: ~s" fn))
  (setf (io-class-output-hook <sco-file>) fn)
  (values))

(defparameter *csound* "csound -d -m 0")

(defun play-sco-file (file &rest args)
  (let* ((cmd *csound*)
         (dir (or (filename-directory file) ""))
         (orc (getf args ':orchestra))
         (snd (getf args ':output))
         (tyo (getf args ':verbose))
         (wai (getf args ':wait (if snd t nil)))
         (pla (getf args ':play (if snd t nil))))
    (cond ((not orc) nil)
          (t
           (unless (filename-directory orc)
             (setf orc (concatenate 'string dir orc)))
           (when snd
             (unless (filename-directory snd)
               (setf snd (concatenate 'string dir snd)))
             (setf wai t)
             (setf cmd (concatenate 'string cmd " -o " snd)))
           (setf cmd (concatenate 'string cmd " " orc " " file))
           (if tyo (format t "~%; ~a" cmd)) (shell cmd :wait wai)
           (if pla (play snd :wait nil)) file))))

(set-sco-output-hook! #'play-sco-file)

(defmethod initialize-io ((io sco-file))
           (format (io-open io) "; ~a output on ~a~%" (cm-version)
                   (date-and-time))
...
(defobject i (event) (ins dur) (:event-streams sco-file))

(defobject f (event) (num size gen) (:event-streams sco-file))

(defmethod sco-name ((obj i))
           (if (slot-boundp obj 'ins)
               (format nil "i~a" (slot-value obj 'ins))
               (let ((n (symbol-name (class-name (class-of obj)))))
                 (if (lower-case-p (elt n 0)) n
                     (string-downcase n)))))

(defmethod sco-name ((obj f))
...
(defun carry-pars (pars last)
  (let ((data pars) (head last) (pnum 1))
    (if (null last)
        (loop for p in data for i from 1 collect
              (if
               (or (string= p ".") (string= p "+")
                   (char= (elt p 0) #\^))
               (error "No p~a value to carry in ~s" i pars)
               (string-read p)))
        (do ()
            ((or (null data) (null last))
             (if (and (null data) (not (null last)))
                 (append pars last) pars))
          (cond ((string= (car data) ".") (rplaca data (car last)))
                ((string= (car data) "+")
                 (unless (= pnum 2)
                ...
                (t (rplaca data (string-read (car data)))
  ...
  (let ((pars (list)))
    ...
    (if (null pars)
        (setf pars
                (copy-list
                 (if (null last)
                     (error "Dangling i_statement: ~a" line) last)))
        (if (not (null last))
            (let ((p (car pars)))
              (when
                  (or (digit-char-p (elt p 0))
                      (and (char= (elt p 0) #\-)
              ...
              (if (not (null last))
                  ...
                  (progn
                   (setf pars (carry-pars pars (list)))
  ...
  (let ((i (gensym)))
    `(let ((,i (floor (car ,pars))))
       (unless (assoc ,i ,d)
         (push
          (list ,i (length ,pars)
                (intern (string-upcase (format nil "i~a" ,i)) :cm))
          ,d)))))

(defmethod import-events ((io sco-file) &key output)
           (let ((secs 0)
                 (rate 60)
                 (beat 0)
                 (head (list))
             ...
             (with-open-io (input io :input)
              (setf in (io-open input))
              (do ()
                  (stop nil)
                (if next (setf line next)
                    (let ((raw (file-line in)))
                      (if (file-eof? raw) (setf line raw)
                          (let ((pos
                                 (do ((i 0 (+ i 1))
                                      (e (length raw))
                                      (f nil))
