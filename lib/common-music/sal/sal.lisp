...
(defmethod emit ((unit list-unit) &optional info errf)
  (let ((lev (get-emit-info ':list info)) ; t if we are sublist
	...
	(if (null sub)
	    (list 'list)
	    `(quote ,(emit sub (add-emit-info ':list t info) errf)))
...
(defun emit-make (form info errf)
  (let ((type (second form)))
	...
	(let* ((class (emit type info errf))
	       ...
	       (extern (list 'make-instance `(quote ,(class-name class)))))
...
(defun parse-stepping (args errf)
  ...
  (cond ((or (token=? (first args) ':repeat)
	...
	(t
	 ...
	 (loop for x in args if (consp x)
	    nconc x else if (not (null x)) collect x))))
...
(defgrammer statement
  `(
    (:print-statement (:print :sexpr (* :co :sexpr))
     ,(lambda (a e) (restate* 'sal-print a e)))
    (:open-statement (:open :sexpr (* :co :key :sexpr))
     ,(lambda (a e) (restate** 'sal-open a e)))
    ...
    (:load-statement (:load :sexpr) 
     ,(lambda (a e) (restate 'sal-load a e)))
    (:system-statement (:system :sexpr (* :co :key :sexpr))
     ,(lambda (a e) (restate** 'sal-system a e)))
    (:chdir-statement (:chdir :sexpr)
     ,(lambda (a e) (restate 'sal-chdir a e)))
    (:play-statement (:play :sexpr) )
    (:rts-statement (:rts :sexpr)
     ,(lambda (a e) (restate 'sal-rts a e)))
...
;; sal-reeval re-evaluation when Enter is
;; pressed multiple times without any intervening editing or cursor
;; motion taking place.

(defun sal-reeval ()
  (if (and *sal-sexpr* *sal-eval*)
      (sal-eval *sal-sexpr*))
  (values))
...
#||
(defun sal-lsdir (&optional dir)
  (let ((here (pwd)))
    (if (not dir)
	(sal-message "Directory: ~A" (namestring here))
	(do ((tail (directory (user-directory dir)) (cdr tail))
	     (file nil))
	    ((null tail)
	     nil)
	  (setq file (enough-namestring (car tail) here))
	  (if (find-if (lambda (c) (char= c #\space)) file)
	      (format t "~&~c~S" #\tab file)
	      (format t "~&~c~A" #\tab file))))
    (values)))
||#
  ...
  (let ((files (list))
	...
	((null tail) )
      ...
      (cond ((not type)
	     (load file))
	    ((string= type "ins")
	     (if (member ':clm *features*)
		 (cload file)
		 (progn ;(setq flag nil)
			(sal-message "CLM is not loaded"))))
	    ((string= type "sal")
	     (load-sal-file file))
	    (t
	     (if bang
