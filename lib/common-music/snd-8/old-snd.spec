...
%description
Snd is a sound editor. It can accommodate any 
number of sounds each with any number of channels, and can be customized and extended
using Guile, Ruby, Gauche, or Forth. Snd is free (LGPL); the code is available via
...
%ifos Linux
%ifarch i386
./configure --prefix=%{prefix} --with-static-xm --with-just-gl CFLAGS="-I/usr/X11R6/include" LDFLAGS="-L/usr/X11R6/lib"
make
%else
...
#   make up-to-date snd-8.tar.gz (using snd-8 dir)
...
#   update version references in /usr/ccrma/web/html/software/snd/index.html, snd.h, sourceforge index.html, configure.ac, NEWS
