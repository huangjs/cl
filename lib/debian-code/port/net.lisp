...
;                vector-to-ipaddr))
#+(or (and sbcl db-sockets) (and sbcl (not (or db-sockets net.sbcl.sockets))))
            ...
            (string
             (if (every (lambda (ch) (or (char= ch #\.) (digit-char-p ch)))
                 ...
                 (let ((hostent
                   ...
                   (when hostent
                     (vector-to-ipaddr
                      (sb-bsd-sockets::host-ent-address hostent))))))
            (integer host)))
         (name
          (when ipaddr
            (let ((hostent
              ...
              (when (and hostent
                         (sb-bsd-sockets::host-ent-aliases hostent))
                (first (sb-bsd-sockets::host-ent-aliases hostent)))))))
...
Returns a socket stream or NIL."
  ...
  #+(and sbcl (not (or db-sockets net.sbcl.sockets)))
  (let ((new-connection (sb-bsd-sockets:socket-accept serv)))
...
(defun socket-server-close (server)
  ...
  #+(and sbcl (not (or db-sockets net.sbcl.sockets))) (sb-bsd-sockets:socket-close server)
  #+(and sbcl net.sbcl.sockets) (close server)
  #-(or abcl allegro clisp cmu gcl lispworks openmcl
        (and sbcl (or net.sbcl.sockets db-sockets (not (or db-sockets net.sbcl.sockets)))) scl)
...
whichever comes first. If there was a timeout, return NIL."
  ...
  #+(and sbcl (or (not (or db-sockets net.sbcl.sockets)) db-sockets))
...
Kind can be :stream or :datagram."
  ...
  #+(and sbcl (not (or db-sockets net.sbcl.sockets)))
  (let ((socket (make-instance 'sb-bsd-sockets:socket :type :stream)))
