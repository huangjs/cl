...
;;; call-fn was called or not. The "id" parameter is there just
;;; to make it easier to tell which tests failed, if any.

(define-test memoize
  (let ((called nil))
    (labels ((call-fn (x) (setq called t) x)
             (called-p (id fn x)
               (setq called nil)
               (funcall fn x)
               called))
      (let ((memo-fn (memoize #'call-fn)))
        ...
        (assert-true (called-p 1 memo-fn 'x))
        (assert-true (called-p 2 memo-fn '(a b)))
        (assert-true (called-p 3 memo-fn nil))

        ;; no calls with old values
        (assert-false (called-p 4 memo-fn 'x))
        (assert-false (called-p 5 memo-fn '(a b)))
        (assert-false (called-p 6 memo-fn nil))
        ...
        (let ((memo-fn2 (memoize #'call-fn)))
          (assert-true (called-p 7 memo-fn2 'x))
          (assert-true (called-p 8 memo-fn2 '(a b)))
          (assert-true (called-p 9 memo-fn2 nil))

          ;; clear the internal table
          (assert-equal nil (funcall memo-fn))
          
          ;; x, (a b) and nil should be new in cleared function
          (assert-true (called-p 10 memo-fn 'x))
          (assert-true (called-p 11 memo-fn '(a b)))
          (assert-true (called-p 12 memo-fn nil))
        
          ;; still old in other function
          (assert-false (called-p 13 memo-fn2 'x))
          (assert-false (called-p 14 memo-fn2 '(a b)))
          (assert-false (called-p 15 memo-fn2 nil))
          )))))
